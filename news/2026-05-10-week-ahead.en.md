...
description: "The European Parliament's Strasbourg plenary of 18–21 May 2026 arrives at a decisive moment for European integration."
...
```mermaid
graph TD
    A[EP Plenary May 18-21] --> B[5 Intelligence Threads]
    B --> T1[T1: Coalition Pressure<br/>EPP 183 seats anchor]
    B --> T2[T2: Digital Leadership<br/>DMA + AI Act]
    B --> T3[T3: Defence Integration<br/>REARM EU nexus]
    B --> T4[T4: Global Relations<br/>Ukraine + Trade]
    B --> T5[T5: Economic Governance<br/>Budget 2027 signal]
    T1 --> C[Centre Coalition 396 seats]
    T2 --> D[IMCO/ITRE Committee leads]
    T3 --> E[AFET/SEDE coordination]
    T4 --> F[Cross-party consensus]
    T5 --> G[BUDG/ECON lead]
    style A fill:#1a7abf,color:#fff
...
```mermaid
graph LR
    EPP[EPP 183 seats] -->|Lead| CC[Centre Coalition 396 seats]
    SD[S&D 136 seats] -->|Partner| CC
    Renew[Renew 77 seats] -->|Pillar| CC
    CC -->|Majority above 360| Legislation

    PfE[PfE 85 seats] -->|Opposition| OPP[Opposition 193 seats]
    ECR[ECR 81 seats] -->|Opposition| OPP
    ESN[ESN 27 seats] -->|Minor opp| OPP

    Greens[Greens 53 seats] -->|Progressive supplement| CC
    Left[Left 45 seats] -->|Issue-based| CC

    Metsola[EP President Metsola] -->|Chairs| Session[Plenary Session]
    Commission[Commission] -->|Legislative initiative| Session
    Council[Council/Poland] -->|Trilogue| Session

    style CC fill:#1a7abf,color:#fff
    style OPP fill:#dc3545,color:#fff
    style EPP fill:#1a7abf,color:#fff
    style SD fill:#e63946,color:#fff
    style PfE fill:#6f42c1,color:#fff
```
...
```mermaid
graph LR
    subgraph Driving["Driving Forces (Score: 31)"]
        D1[Coalition Agreement ★★★★★]
        D2[EPP Institutional Stakes ★★★★★]
        D3[Electoral Deterrent ★★★★☆]
        D4[Security Consensus ★★★★☆]
        D5[Renew Survival ★★★☆☆]
    end

    subgraph Restraining["Restraining Forces (Score: 21)"]
        R1[EPP Right-Flank Magnetism ★★★★☆]
        R2[Populist Narrative ★★★★☆]
        R3[S&D Base Demands ★★★☆☆]
        R4[Renew Fragmentation ★★★☆☆]
        R5[Coalition Fatigue ★★★☆☆]
    end

    Driving -->|Net +10| Outcome[Coalition Holds]
    Restraining -->|Opposes| Outcome

    style Outcome fill:#28a745,color:#fff
    style Driving fill:#d4edda
    style Restraining fill:#f8d7da
```
...
```mermaid
quadrantChart
    title Session Heatmap — Impact vs. Probability
    ...
    Wednesday Vote Block: [0.90, 0.80]
    Coalition Fracture: [0.25, 0.95]
    External Crisis: [0.12, 0.90]
    Digital Policy Signal: [0.60, 0.75]
    Media Narrative: [0.80, 0.50]
    QT Accountability: [0.95, 0.40]
    Budget Signal: [0.50, 0.60]
```
...
```mermaid
graph LR
    subgraph Centre["Centre Coalition (396 seats ✓ > 360 majority)"]
        EPP[EPP 183]
        SD[S&D 136]
        Renew[Renew 77]
    end

    subgraph Supplement["Progressive Supplement (case-by-case)"]
        Greens[Greens 53]
        Left[Left 45]
    end

    subgraph Opposition["Opposition (193 seats)"]
        PfE[PfE 85]
        ECR[ECR 81]
        ESN[ESN 27]
    end

    NI[NI 30] -.->|Varies| Centre
    Centre -->|396 seats| Majority[Majority Threshold 360]
    Supplement -.->|Issue-specific| Majority

    style Centre fill:#1a7abf,color:#fff
...
```mermaid
graph TD
    EPP[EPP 183 seats] --> CC[Centre Coalition]
    SD[S&D 136 seats] --> CC
    Renew[Renew 77 seats] --> CC
    CC --> Votes[~17 Plenary Votes]

    PfE[PfE 85] --> Opp[Opposition]
    ECR[ECR 81] --> Opp
    Greens[Greens 53] -.->|Progressive supplement| CC
    Left[Left 45] -.->|Issue-based| CC

    Metsola[Metsola - EP President] --> Session[May Plenary]
    Commission[Commission] --> Session
    Session --> Votes

    Citizens[448M EU Citizens] --> Represented[Democratic Representation]
    Votes --> Represented
...
```mermaid
graph TD
    A[EU Economic Policy Context] --> B[EU Budget 2027]
    A --> C[Green Transition]
    A --> D[Digital Economy]
    A --> E[Trade Policy]
    A --> F[Defence Investment]
    B --> G[BUDG Committee]
    C --> H[ITRE/ENVI]
    D --> I[IMCO/ITRE]
    E --> J[INTA Committee]
    ...
    note[IMF DATA UNAVAILABLE - EP source only]
```
...
quadrantChart
    ...
    x-axis Rare --> Very Likely
    y-axis Negligible --> Critical
    quadrant-1 High Risk
    quadrant-2 Critical Monitor
    quadrant-3 Low Risk
    quadrant-4 Routine

    Coalition Defection R01: [0.25, 0.70]
    External Crisis R02: [0.12, 0.90]
    Coalition Majority Failure R03: [0.05, 0.95]
    Cyber Attack R04: [0.03, 0.75]
    Populist Narrative R05: [0.60, 0.55]
    IMF Data Absent R06: [0.95, 0.30]
    Disinformation R08: [0.35, 0.30]
```
...
xychart-beta
    ...
    x-axis ["S1 Coalition", "S2 Volume", "S3 Metsola", "S4 Digital", "S5 Credibility", "W1 IMF", "W2 Votes", "W3 Titles", "W4 Buffer", "W5 Events", "O1 Convergence", "O2 EPP", "O3 Ukraine", "O4 AI Act", "O5 Bridge", "T1 Defection", "T2 Narrative", "T3 Crisis", "T4 Fatigue", "T5 Disinfo"]
    y-axis "Score (Magnitude × Certainty)" 0 --> 30
    bar [25, 20, 16, 12, 12, 15, 15, 15, 12, 10, 15, 12, 16, 12, 9, 12, 12, 10, 9, 6]
```
...
```mermaid
graph TD
    A[May 18-21 Session] --> B[Coalition Integrity]
    A --> C[External Environment]
    A --> D[Information Domain]
    A --> E[Institutional Process]

    B --> B1[EPP Right-Flank Defection<br/>WEP: Unlikely 25%]
    B --> B2[Coalition Majority Failure<br/>WEP: Highly Unlikely 5%]

    C --> C1[Russia-Ukraine Escalation<br/>WEP: Very Unlikely 12%]
    C --> C2[Cyber Attack<br/>WEP: Highly Unlikely 3%]

    D --> D1[Disinformation Campaign<br/>WEP: Roughly Even 40%]
    D --> D2[Narrative Spoofing<br/>WEP: Likely 60%]

    E --> E1[Procedural Manipulation<br/>WEP: Unlikely 20%]
    E --> E2[Repudiation / Roll-call evasion<br/>WEP: Roughly Even 40%]
...
```mermaid
graph LR
    subgraph High["High Priority Threats"]
        T1[EPP Right-Flank Defection<br/>WEP: Unlikely 25%]
        T2[External Crisis Disruption<br/>WEP: Very Unlikely 12%]
    end

    subgraph Medium["Medium Priority Threats"]
        T3[Populist Narrative Capture<br/>WEP: Likely 60%]
        T4[Coalition Fatigue<br/>WEP: Roughly Even 40%]
    end

    subgraph Low["Low Priority Threats"]
        T5[Cyber Attack<br/>WEP: Highly Unlikely 3%]
        T6[Coalition Collapse<br/>WEP: Highly Unlikely 5%]
    end

    High -->|Monitor| Response[EP Institutional Response]
    Medium -->|Track| Response

    style High fill:#FFC107
    style Medium fill:#17A2B8,color:#fff
...
pie title Scenario Probability Distribution
    "Scenario 1 – Normal Session" : 60
    "Scenario 2 – Contested Vote" : 20
    "Scenario 3 – Progressive Push" : 10
    "Scenario 4 – External Emergency" : 10
```
...
```mermaid
xychart-beta
    title "7-Day Horizon Outcome Probability Distribution"
    x-axis ["Normal (A)", "Normal+Urgency (B)", "Contested Vote (C)", "Coalition Stress (D)", "Crisis/Emergency"]
    y-axis "Probability (%)" 0 --> 80
    bar [60, 15, 20, 4, 1]
```
...
```mermaid
graph LR
    subgraph PESTLE["PESTLE Analysis — May 2026"]
        P[Political<br/>Coalition pressure; right-rise]
        E[Economic<br/>MFF 2027; trade tensions]
        S[Social<br/>Migration debate; youth]
        T[Technological<br/>DMA, AI Act, digital]
        L[Legal<br/>CJEU; rule of law; Charter]
        En[Environmental<br/>Green Deal; taxonomy]
    end

    PESTLE --> Session[May 18-21 Session Impact]
    P --> Session
    E --> Session
    T --> Session
    L --> Session
...
```mermaid
timeline
    title EP Coalition Strength Over Time
    EP7 (2009-14) : Grand Coalition dominant
                  : ~70% majority
    EP8 (2014-19) : EPP+S&D+Liberals
                   : ~65% majority
    EP9 (2019-24) : EPP+S&D+Renew
                  : ~60% majority
                  : Right-wing growing
    EP10 (2024-29) : EPP+S&D+Renew
                   : 396/720 = 55%
                   : ENP 6.58 (high fragmentation)
                   : May 2026 = Year 2
```
...
```mermaid
graph TD
    Session[May 18-21 EP Session] --> F1[Frame 1: Coalition Under Pressure 60%]
    Session --> F2[Frame 2: Europe as Global Actor 50%]
    Session --> F3[Frame 3: Democratic Accountability 40%]
    Session --> F4[Frame 4: Regulation vs. Innovation 35%]
    Session --> F5[Frame 5: Urgency/Crisis 15%]

    F1 --> M1[Politico; Euractiv; Le Monde]
    F2 --> M2[FT; Economist; SZ]
    F3 --> M3[DW; Euronews; National broadcast]
    F4 --> M4[Bloomberg; Reuters; WSJ]
    F5 --> M5[Breaking news; social media]

    style F1 fill:#FFC107,color:#000
    style F2 fill:#28a745,color:#fff
    style F3 fill:#17a2b8,color:#fff
...
| section-quality-reflection | [methodology-reflection](https://github.com/Hack23/euparliamentmonitor/blob/main/analysis/daily/2026-05-10/week-ahead/methodology-reflection.md) | `methodology-reflection.md` |
