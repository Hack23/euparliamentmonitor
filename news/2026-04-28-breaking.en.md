---
title: "EP Breaking: Strasbourg April Plenary & US — Run 1777360024"
description: "The European Parliament is in its April 2026 Strasbourg plenary session (April 27–30) for democratic-accountability readers tracking EU institutional consequences."
...
source_folder: analysis/daily/2026-04-28/breaking
...
| [Supplementary intelligence](#section-supplementary-intelligence) | additional markdown discovered in the run that has not yet been assigned to a canonical section | `intelligence/geopolitical-risk.md` |
...
xychart-beta
    title "Force Field Balance — April 2026"
    x-axis ["Trade Defence", "Banking Reform", "Anti-Corruption", "EU-Canada", "Ukraine"]
    y-axis "Net Force Score (Driving minus Restraining)" 0 --> 10
    bar [6.5, 5.0, 4.5, 5.5, 7.0]
```
...
%%{init: {"theme":"dark","themeVariables":{"primaryColor":"#1565C0","primaryTextColor":"#ffffff","lineColor":"#90CAF9"}}}%%
quadrantChart
    title Stakeholder Impact Map (Influence vs Net Impact)
    x-axis Low Influence --> High Influence
    y-axis Negative Impact --> Positive Impact
    quadrant-1 High Influence, Positive Impact
    quadrant-2 Low Influence, Positive Impact
    quadrant-3 Low Influence, Negative Impact
    quadrant-4 High Influence, Negative Impact
    EU Commission: [0.9, 0.8]
    Member States: [0.8, 0.7]
    EU Civil Society: [0.3, 0.85]
    Ukrainian Citizens: [0.2, 0.9]
    EU Consumers: [0.4, 0.55]
    EU Manufacturers: [0.6, 0.4]
    EU Banks: [0.7, 0.35]
    US Exporters: [0.5, 0.1]
    Far-right MEPs: [0.3, 0.2]
```
...
```mermaid
graph LR
    EPP[EPP<br/>Capital: HIGH<br/>Risk: LOW]
    SD[S&D<br/>Capital: HIGH<br/>Risk: LOW]
    Renew[Renew<br/>Capital: MEDIUM<br/>Risk: MEDIUM-HIGH]
    ECR[ECR<br/>Capital: MEDIUM<br/>Risk: MEDIUM]
    PfE[PfE<br/>Capital: LOW-MEDIUM<br/>Risk: HIGH]

    EPP -->|Coalition lead| SD
    SD -->|Progressive bloc| Renew
    ECR -.->|Tactical support| EPP
    PfE -->|Opposition| ECR
```
...
```mermaid
flowchart TD
    Normal[Normal Legislative Process] --> Procedural{Procedural<br/>Disruption?}
    Procedural -->|Roll-call abuse| Delay[15-30 min delay<br/>per vote]
    Procedural -->|No| Political{Political<br/>Disruption?}
    Political -->|Coalition fracture| Gridlock[Temporary<br/>Gridlock]
    Political -->|No| External{External<br/>Event?}
    External -->|Geopolitical crisis| Emergency[Emergency<br/>Plenary]
    External -->|No| Legal{Legal<br/>Challenge?}
    Legal -->|Court blocks| Restart[Legislative<br/>Restart]
    Legal -->|No| Adoption[Normal<br/>Adoption]

    style Emergency fill:#ff6600
    style Gridlock fill:#ffcc00
    style Restart fill:#ff6600
```
...
```mermaid
%%{init: {"theme":"dark","themeVariables":{"primaryColor":"#1565C0","primaryTextColor":"#ffffff","lineColor":"#90CAF9"}}}%%
pie title EU Parliament Scenario Distribution (6-18 month horizon)
    "Managed Trade Tension" : 57
    "Trade War Escalation" : 32
    "Transatlantic Reset" : 15
    "Coalition Fracture" : 20
    "Wildcard" : 13
```
