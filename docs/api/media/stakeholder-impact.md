...
| **Evidence Sources** | `[REQUIRED: EP MCP data + IMF indicators (primary for economic impact) + optional WB non-economic indicators]` |
| **Confidence Level** | `[REQUIRED: HIGH / MEDIUM / LOW]` |
...
> **Note:** IMF is the required primary source for every economic citation in this section. Any projected number must include a forecast marker (`forecast`/`projection`/`projects`/`expects`) within 30 words, plus the vintage in prose (`IMF WEO April 2026`), plus `data-vintage="WEO-April-2026"` on the enclosing `<section>` element.
>
