...
**Server:** `european-parliament-mcp-server@1.3.9`
...
### DOCEO Near-Realtime Vote Data (v1.3.9+)
...
> **✅ v1.3.7 – v1.3.9 OSINT upgrades (active on the pinned `v1.3.9` server):**
...
> | `detect_voting_anomalies` | DOCEO-backed with z-score baselines for defection / abstention / attendance spikes. Severity classification (HIGH/MEDIUM/LOW) is now statistically grounded. | [#476](https://github.com/Hack23/European-Parliament-MCP-Server/pull/476) (v1.3.9) |
...
> pinned to `v1.3.9`); explicit dates remain the required Stage-A pattern
...
6. Re-classify obsolete findings: item #1 is **resolved upstream in v1.2.15+** — the gateway is now pinned to v1.3.9, so it should no longer surface in audits. Item #2's alias-fragmentation symptom is similarly suppressed in v1.2.15+ but the canonical-short-code consumer rule remains the primary triage answer at every gateway version.
