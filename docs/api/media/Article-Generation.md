...
**📋 Document Owner:** CEO | **📄 Version:** 1.3 | **📅 Last Updated:** 2026-05-29 (UTC)
...
> **Localized-brief-first body contract (2026-05).** Every non-English HTML variant **always opens with the executive brief in the reader's own language when a translated `executive-brief_<lang>.md` sibling exists, and with the English brief otherwise.** When the run does not ship a full per-language `article.<lang>.md` source, `writeLanguageVariant` in [`src/aggregator/generator/render-one.ts`](./src/aggregator/generator/render-one.ts) reads the translated brief body via `readLocalizedBriefBody(runDir, lang)` and splices its rendered markdown into the `#section-executive-brief` block with `replaceExecutiveBriefSection(...)` ([`src/aggregator/html/localize-body.ts`](./src/aggregator/html/localize-body.ts)). The splice's boundary detection (`findExecutiveBriefSectionCut`) treats any `id="section-…"` heading **and** the Reader-Guide / Tradecraft / Analysis-Index / Supplementary appendix anchors as the end of the brief, and extends to end-of-body when the brief is the last block — so the localized brief is injected even on **sparse runs** where the Executive Brief is the only canonical `section-` heading. The brief's own internal `<h2>` sub-headings (`## BLUF`, `## 60-Second Read`) are slugified anchors without the `section-` prefix and are therefore never mistaken for the boundary. SEO surfaces (`<title>`, `<meta description>`, JSON-LD `headline`) are localized separately and earlier via `resolveLocalizedBriefHighlight`. Unit coverage: `test/unit/localize-body-executive-brief.test.js`.
...
> **Localized-brief precedence (resolver extension — shipped).** Tier 2 now prefers a translated `executive-brief_<lang>.md` over the English `executive-brief.md` for each non-English language whenever the manifest carries no override for `<lang>`. `resolveArticleMetadata` passes `resolveLocalizedBrief: resolveLocalizedBriefHighlight` into the per-language resolver ([`src/aggregator/metadata/per-language-resolver.ts`](./src/aggregator/metadata/per-language-resolver.ts)); when a localized brief yields a headline or summary it is tagged with the `localized-brief` source and wins over the English editorial fallback. Stage-B agents may still set `manifest.title.<lang>` / `manifest.description.<lang>` explicitly (Tier 1) to override this — the manifest pack always takes precedence over both the localized and English briefs.
