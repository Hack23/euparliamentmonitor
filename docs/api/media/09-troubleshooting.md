...
| `INTERNAL_ERROR` | MCP server internal failure — verify `european-parliament-mcp-server@1.3.14` installed; check DNS for `data.europarl.europa.eu`. |
...
| **Agent crashed before `git checkout -b news/...` or before `git commit`** — Stage B artifacts exist on disk under `analysis/daily/${DATE}/${SLUG}/` but no `news/*` branch was ever created. The old `gh-aw-capture-agent-patch.sh` exited silently (`no news/* branch ahead of origin/main; nothing to capture`) and the entire run's work was lost when the runner was reclaimed. | The capture script's `git for-each-ref refs/heads/news/` discovery returns empty, so the `aw-agent-recovery.patch` was never written. Agents that crash early (process-OOM, MCP gateway timeout during inference, Copilot 429 reflection) commonly leave untracked artifacts in this state. | The capture script now has a **crash-recovery fallback path**: when no news/* branch ahead of origin/main is found, it scans for uncommitted changes restricted to `analysis/daily/` and `news/`, stages them, emits a synthetic `aw-agent-recovery.patch` from the staged diff, then unstages so the workspace state is preserved. The patch carries `recovery_kind=crash-recovery` and `commits_ahead=0` in the `.meta` sidecar so debug tooling can distinguish it from a normal news-branch capture. The existing PAT fallback applies this patch via its `aw-*.patch` glob with no PAT-fallback changes required. Drift-guarded by `test/unit/gh-aw-capture-agent-patch.test.js` ("captures uncommitted analysis/daily/ artifacts when no news/* branch exists"). |
