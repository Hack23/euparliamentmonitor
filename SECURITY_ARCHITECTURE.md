...
**EU Parliament Monitor v0.8.40 (2026-04-20)** publishes **1894 static HTML articles** across **14 languages** and **7 article types** (breaking, week-ahead, week-in-review, month-ahead, month-in-review, committee-reports, motions, propositions), generated by **10 gh-aw agentic workflows** and validated by **3061+ automated tests** across **52 test files**. Primary delivery is **AWS S3 + CloudFront** via OIDC federation; GitHub Pages remains the documented fallback. Release artifacts are signed with **npm provenance** and **SLSA Level 3 build attestations**. **OpenSSF Scorecard + OpenSSF Best Practices #12068** certify the posture.
...
1. **`gh aw compile --validate`** (pinned to **v0.68.7**):
...
- **Rate Limiting**: Client-side throttling (max 10 requests/minute) to respect IMF service limits

**Security Controls**:
- **Certificate Validation**: Node.js built-in TLS certificate chain validation
- **HTTPS-Only**: HTTP requests rejected by schema validation
- **Response Validation**: SDMX 3.0 schema validation for all responses
- **Error Handling**: Network failures trigger fallback to cached/sample data
...
1. **Canonical Tool Lists**:
   - `EP_MCP_TOOLS`: Expected tools from European Parliament MCP server
   - `IMF_MCP_TOOLS`: Expected economic data endpoints
   - `WORLD_BANK_MCP_TOOLS`: Expected World Bank data tools

2. **Integration Tests**:
   - `test/integration/mcp-drift-detection.test.ts` asserts exact tool list matches
   - Any upstream tool rename, addition, or removal = **test failure**
   - Blocks PR merge until drift is reviewed and tool lists updated
...
```json
{
  "mcpServers": {
    "european-parliament": {
      "command": "npx",
      "args": ["-y", "european-parliament-mcp-server@1.2.10"],
      "env": {
...
CSP headers are set via **CloudFront response-headers policy** (not meta tags):

- `default-src 'self'`
- `script-src 'self' 'unsafe-inline'` — needed for Chart.js inline init; external `js/index-runtime.js` + `js/article-runtime.js`
- `connect-src 'self'`
- `img-src 'self' data: https:`
- `style-src 'self' 'unsafe-inline'` — inline CSS variables
- **HSTS**: Strict-Transport-Security enforced
- **X-Content-Type-Options**: nosniff
- **Referrer-Policy**: strict-origin-when-cross-origin
- **Permissions-Policy**: Restrictive via CloudFront
