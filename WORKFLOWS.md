...
graph TD
    ...
    H --> I[🔗 Install EP MCP Server v1.3.9+]
...
| **Node.js version** | 26 |
| **EP MCP Server** | `european-parliament-mcp-server@1.3.9` (globally installed via `scripts/mcp-setup.sh`, MCP gateway `EP_MCP_GATEWAY_URL` resolved dynamically from MCP config; default `http://host.docker.internal:8080/mcp/european-parliament`) |
...
```mermaid
flowchart TD
    subgraph Collection["📡 COLLECTION<br/>(EP MCP Server v1.3.9+)"]
        direction TB
...
flowchart LR
    ...
    subgraph "🔌 MCP Layer"
        MCP["EP MCP Server<br/>v1.3.9+<br/>(180s timeout;<br/>60+ tools, sliding + fixed-window feeds)"]
    end
