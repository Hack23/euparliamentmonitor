...
export function buildSiteHeader(options: SiteHeaderOptions): string {
  ...
  const cta = (extraClass: string, href: string, iconName: IconName, label: string): string =>
    `<a class="site-header__cta${extraClass ? ` ${extraClass}` : ''}" href="${href}" rel="noopener" aria-label="${label}" title="${label}">${icon(iconName)}<span class="site-header__cta-label">${label}</span></a>`;
