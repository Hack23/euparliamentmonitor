...
function buildStrideResult(
  ...
  lowAnalysis: string,
  elevatedAnalysis: (level: string) => string
): PoliticalStrideCategory {
 ...
 */
export function assessPoliticalThreats(data: ArticleData): PoliticalThreatAssessment {
  ...
  const hasHighEvidence =
    actorProfiles.length > 0 || strideCategories.some((c) => c.evidence.length > 1);
  const confidence: 'high' | 'medium' | 'low' = hasHighEvidence ? 'medium' : 'low';
 ...
 */
export function buildConsequenceTree(action: string, data: ArticleData): PoliticalConsequenceTree {
  const safeAction =
  ...
  const immediateConsequences: ConsequenceNode[] = [
    {
      ...
      affectedStakeholders: ['political_group', 'eu_institution'] as PoliticalActorType[],
      timeframe: 'immediate',
    },
    {
      ...
      affectedStakeholders: ['political_group'] as PoliticalActorType[],
      timeframe: 'immediate',
    },
  ];
  ...
  const secondaryEffects: ConsequenceNode[] = [
    {
      ...
      probability: 0.5,
      impact: 'medium',
      affectedStakeholders: ['civil_society', 'industry', 'member_state'] as PoliticalActorType[],
    ...
    },
    {
      ...
      affectedStakeholders: ['political_group', 'mep'] as PoliticalActorType[],
      timeframe: 'short-term',
    },
  ];
  ...
  const longTermImplications: ConsequenceNode[] = [
    {
      ...
      probability: 0.4,
      impact: 'medium',
      affectedStakeholders: ['eu_institution', 'political_group'] as PoliticalActorType[],
      timeframe: 'long-term',
    },
    {
 ...
 */
export function generateThreatAssessmentMarkdown(assessment: PoliticalThreatAssessment): string {
  const lines: string[] = [
    '---',
  ...
  assessment.legislativeDisruptions.forEach((analysis) =>
