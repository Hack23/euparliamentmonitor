 ...
 */
export function articlePolicyHasIMFEconomicEvidence(
  html: string,
  articleType: string
): boolean {
  if (!POLICY_SLUGS_REQUIRING_WORLD_BANK.has(articleType)) return true;
 ...
 */
export function isWave3IMFStrictEnabled(
  env: Readonly<Record<string, string | undefined>> = process.env
): boolean {
  // Highest-precedence: legacy escape hatch wins outright.
  if (parseTruthyFlag(env['WAVE3_IMF_LEGACY'])) return false;
  ...
  if (typeof strictRaw === 'string' && isExplicitlyFalsy(strictRaw)) return false;
  return true;
}
 ...
 */
function isExplicitlyFalsy(raw: string): boolean {
  const normalised = raw.trim().toLowerCase();
