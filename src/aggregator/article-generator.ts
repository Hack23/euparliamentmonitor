...
export interface CliOptions {
  ...
  /** Optional: override the auto-derived article title. */
  readonly title?: string;
  /** Optional: override the auto-derived article description. */
...
export function parseCliArgs(argv: readonly string[], repoRoot: string): CliOptions {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (result.kind === 'runDir') runDir = result.value;
    else if (result.kind === 'lang') langs.push(result.value);
    else if (result.kind === 'outDir') outDir = result.value;
    else if (result.kind === 'title') title = result.value;
    else if (result.kind === 'description') description = result.value;
    else if (result.kind === 'markdownOnly') markdownOnly = true;
  }
  if (!runDir) {
 ...
 */
export function buildArticleSlug(date: string, articleType: string): string {
 ...
 */
export function generateArticle(opts: CliOptions): GenerateResult {
