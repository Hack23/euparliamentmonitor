...
function resolveFinalTitle({
  ...
  readonly input: PerLanguageInputs;
  readonly nonLatinFamily: boolean;
  readonly perLanguageSource: ResolvedMetadataEntry['source'];
  ...
  const usedContextualFallback =
    truncatedTitle === truncateTitle(contextualFallback) || truncatedTitle === contextualFallback;
  if (
    nonLatinFamily &&
...
function resolveTruncatedDescription(
  lang: LanguageCode,
  ...
  return padDescriptionToFloor(
    ensureDescriptionTerminator(lang, clampedDescription, terminatorBudget),
    lang
  );
}
...
export function resolveOneLanguage(input: PerLanguageInputs): ResolvedMetadataEntry {
  ...
  let truncatedDescription = resolveTruncatedDescription(
    input.lang,
    normalizedRawDescription,
    description
  );
