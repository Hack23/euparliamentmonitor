 ...
 */
function buildVotingPoliticalImpact(
  recordCount: number,
  adoptedCount: number,
  anomalyCount: number,
  intensity: VotingIntensity | null
...
export function buildProspectiveAnalysis(
  ...
  return {
    ...
    impactAssessment: {
      ...
      economic:
        docCount > 0
          ? `legislative_documents=${docCount}`
          : 'no economic legislation flagged',
      social:
        questionCount > 0
      ...
      legal:
        pipelineCount > 0
          ? `pipeline_procedures=${pipelineCount}`
          : 'no pipeline procedures',
