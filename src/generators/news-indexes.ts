...
function detectCategory(slug: string): string {
  ...
  if (s.includes('week-ahead')) return DEFAULT_CATEGORY;
  if (s.includes('committee')) return 'committee';
  if (s.includes('motion') || s.includes('vote') || s.includes('voting')) return 'motions';
  if (s.includes('propos') || s.includes('legislat')) return 'propositions';
