---
name: "News: EU Parliament Month Ahead — Article"
description: Generates the EU Parliament month ahead English article once the paired analysis PR is merged. Stage D only. Reads committed analysis from main, optional bounded Stage-A top-up, single article PR.
strict: false
on:
  pull_request:
    types: [closed]
  ...
  workflow_dispatch:
    inputs:
      analysis_date:
        ...
        required: false
      article_type:
        description: 'Article type slug. Fixed to "month-ahead" for this workflow.'
        required: false
        default: month-ahead
  steps:
    - name: Check analysis PR trigger gate
      ...
      run: |
        ...
        if ! echo "$LABELS" | grep -q '"type:month-ahead"'; then
          echo "PR missing 'type:month-ahead' label — skipping article workflow"
          exit 1
        fi
...
concurrency:
  group: "news-article-month-ahead"
...
safe-outputs:
  ...
  create-pull-request:
    title-prefix: "[news] "
    labels: [agentic-news, "type:month-ahead"]
    draft: false
    expires: 14d
...
---
# 📰 EU Parliament Month Ahead — Article Workflow
...
`news-month-ahead-analysis.md` workflow.
...
3. The PR carries the `type:month-ahead` label.
...
RUN_EPOCH=$(date -u +%s)
RUN_ID="month-ahead-article-run$$-$RUN_EPOCH"
...
|-----------|-------|
| `ARTICLE_TYPE_SLUG` | `month-ahead` |
| Family | Split: **article-only** (consumes analysis from `news-month-ahead-analysis.md`) |
...
```bash
ARTICLE_HTML=$(ls -t "news/${ANALYSIS_DATE}-month-ahead"*"-en.html" 2>/dev/null | head -1)
[ -n "$ARTICLE_HTML" ] && \
...
- `base: "main"`
- `head: "news/${ANALYSIS_DATE}-month-ahead-${RUN_ID}"`
