---
name: "News: EU Parliament Monthly Review — Article"
description: Generates the EU Parliament monthly review English article once the paired analysis PR is merged. Stage D only. Reads committed analysis from main, optional bounded Stage-A top-up, single article PR.
strict: false
on:
  pull_request:
    types: [closed]
...
concurrency:
  group: "news-article-month-in-review"
...
safe-outputs:
  ...
  create-pull-request:
    title-prefix: "[news] "
    labels: [agentic-news, "type:month-in-review"]
    draft: false
    expires: 14d
...
---
# 📰 EU Parliament Monthly Review — Article Workflow
...
`news-monthly-review-analysis.md` workflow.
...
3. The PR carries the `type:month-in-review` label.
...
RUN_EPOCH=$(date -u +%s)
RUN_ID="month-in-review-article-run$$-$RUN_EPOCH"
...
|-----------|-------|
| `ARTICLE_TYPE_SLUG` | `month-in-review` |
| Family | Split: **article-only** (consumes analysis from `news-monthly-review-analysis.md`) |
...
- Economic context (**IMF primary** for macro/fiscal/monetary/trade — Wave-3 policy; WB only for non-economic) is mandatory. Cross-reference prior month-ahead predictions; produce a confirmed/refuted tally. Include `intelligence/historical-baseline.md` in the analysis set.
...
```bash
ARTICLE_HTML=$(ls -t "news/${ANALYSIS_DATE}-month-in-review"*"-en.html" 2>/dev/null | head -1)
[ -n "$ARTICLE_HTML" ] && \
...
- `base: "main"`
- `head: "news/${ANALYSIS_DATE}-month-in-review-${RUN_ID}"`
