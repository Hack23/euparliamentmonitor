---
name: "News: EU Parliament Breaking News — Article"
...
safe-outputs:
  ...
  create-pull-request:
    title-prefix: "[news] "
    labels: [agentic-news, "type:breaking"]
    draft: false
    expires: 14d
...
`news-breaking-analysis.md` workflow.
...
3. The PR carries the `type:breaking` label.
...
RUN_EPOCH=$(date -u +%s)
RUN_ID="breaking-article-run$$-$RUN_EPOCH"
...
|-----------|-------|
| `ARTICLE_TYPE_SLUG` | `breaking` |
| Family | Split: **article-only** (consumes analysis from `news-breaking-analysis.md`) |
...
feeds with `timeframe=today` and append anything new to
...
```bash
ARTICLE_HTML=$(ls -t "news/${ANALYSIS_DATE}-breaking"*"-en.html" 2>/dev/null | head -1)
[ -n "$ARTICLE_HTML" ] && \
...
- `base: "main"`
- `head: "news/${ANALYSIS_DATE}-breaking-${RUN_ID}"`
