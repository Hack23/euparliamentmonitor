---
name: "News: EU Parliament Monthly Review — Analysis"
description: Produces a single analysis-only PR for EU Parliament monthly review. Stages A + B + C only. Paired with news-monthly-review-article.md, which runs Stage D when this PR merges.
strict: false
on:
  schedule:
    - cron: "0 10 28 * *"
...
concurrency:
  group: "news-analysis-month-in-review"
...
safe-outputs:
  ...
  create-pull-request:
    ...
    labels: [agentic-analysis, analysis-data, "type:month-in-review"]
    draft: false
    expires: 14d
...
---
# 🔬 EU Parliament Monthly Review — Analysis-Only Workflow
...
(article drafting) happens in the paired `news-monthly-review-article.md` workflow
...
| Family | Split: **analysis-only** (paired with `news-monthly-review-article.md`) |
...
Stage D happens in the paired `news-monthly-review-article.md` workflow.
...
- `base: "main"`
- `head: "analysis/${TODAY}-month-in-review-${RUN_ID}"`
- `title: "[analysis] month-in-review — ${TODAY} (run ${RUN_ID})"`
