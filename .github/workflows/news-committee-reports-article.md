---
name: "News: EU Parliament Committee Reports — Article"
description: Generates the EU Parliament committee reports English article once the paired analysis PR is merged. Stage D only. Reads committed analysis from main, optional bounded Stage-A top-up, single article PR.
strict: false
on:
  pull_request:
    types: [closed]
  ...
  workflow_dispatch:
    inputs:
      analysis_date:
        ...
        required: false
      article_type:
        description: 'Article type slug. Fixed to "committee-reports" for this workflow.'
        required: false
        default: committee-reports
  steps:
    - name: Check analysis PR trigger gate
      ...
      run: |
        ...
        if ! echo "$LABELS" | grep -q '"type:committee-reports"'; then
          echo "PR missing 'type:committee-reports' label — skipping article workflow"
          exit 1
        fi
...
concurrency:
  group: "news-article-committee-reports"
...
safe-outputs:
  ...
  create-pull-request:
    title-prefix: "[news] "
    labels: [agentic-news, "type:committee-reports"]
    draft: false
    expires: 14d
...
---
# 📰 EU Parliament Committee Reports — Article Workflow
...
`news-committee-reports-analysis.md` workflow.
...
3. The PR carries the `type:committee-reports` label.
...
RUN_EPOCH=$(date -u +%s)
RUN_ID="committee-reports-article-run$$-$RUN_EPOCH"
...
|-----------|-------|
| `ARTICLE_TYPE_SLUG` | `committee-reports` |
| Family | Split: **article-only** (consumes analysis from `news-committee-reports-analysis.md`) |
...
- Ground every claim in a named committee + document ID (e.g. ENVI draft report on directive 2025/0042(COD)). Include `existing/committee-productivity.md` in the analysis set.
...
```bash
ARTICLE_HTML=$(ls -t "news/${ANALYSIS_DATE}-committee-reports"*"-en.html" 2>/dev/null | head -1)
[ -n "$ARTICLE_HTML" ] && \
...
- `base: "main"`
- `head: "news/${ANALYSIS_DATE}-committee-reports-${RUN_ID}"`
