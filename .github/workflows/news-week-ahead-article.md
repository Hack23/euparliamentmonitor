  ...
  workflow_dispatch:
    inputs:
      analysis_date:
        ...
        required: false
      article_type:
        description: 'Article type slug. Fixed to "week-ahead" for this workflow.'
        required: false
        default: week-ahead
  steps:
    - name: Check analysis PR trigger gate
      ...
      run: |
        ...
        if ! echo "$LABELS" | grep -q '"type:week-ahead"'; then
          echo "PR missing 'type:week-ahead' label — skipping article workflow"
          exit 1
        fi
