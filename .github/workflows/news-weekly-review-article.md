---
name: "News: EU Parliament Weekly Review — Article"
description: Generates the EU Parliament weekly review English article once the paired analysis PR is merged. Stage D only. Reads committed analysis from main, optional bounded Stage-A top-up, single article PR.
strict: false
on:
  pull_request:
    types: [closed]
  ...
  workflow_dispatch:
    inputs:
      analysis_date:
        ...
        required: false
      article_type:
        description: 'Article type slug. Fixed to "week-in-review" for this workflow.'
        required: false
  ...
  steps:
    - name: Check analysis PR trigger gate
      ...
      run: |
        ...
        if ! echo "$LABELS" | grep -q '"type:week-in-review"'; then
          echo "PR missing 'type:week-in-review' label — skipping article workflow"
          exit 1
        fi
...
concurrency:
  group: "news-article-week-in-review"
...
safe-outputs:
  ...
  create-pull-request:
    title-prefix: "[news] "
    labels: [agentic-news, "type:week-in-review"]
    draft: false
    expires: 14d
...
---
# 📰 EU Parliament Weekly Review — Article Workflow
...
`news-weekly-review-analysis.md` workflow.
...
3. The PR carries the `type:week-in-review` label.
...
RUN_EPOCH=$(date -u +%s)
RUN_ID="week-in-review-article-run$$-$RUN_EPOCH"
...
|-----------|-------|
| `ARTICLE_TYPE_SLUG` | `week-in-review` |
| Family | Split: **article-only** (consumes analysis from `news-weekly-review-analysis.md`) |
...
- Cross-reference prior week-ahead predictions — confirm or refute forward statements. Include `intelligence/historical-baseline.md` + `risk-scoring/risk-matrix.md` in the analysis set.
...
```bash
ARTICLE_HTML=$(ls -t "news/${ANALYSIS_DATE}-week-in-review"*"-en.html" 2>/dev/null | head -1)
[ -n "$ARTICLE_HTML" ] && \
...
- `base: "main"`
- `head: "news/${ANALYSIS_DATE}-week-in-review-${RUN_ID}"`
