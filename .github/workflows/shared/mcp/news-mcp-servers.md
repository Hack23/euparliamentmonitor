...
| `fetch-proxy` | inline Node.js MCP server | IMF-only HTTPS fetch proxy — bypasses AWF Squid proxy for `api.imf.org/external/sdmx/` calls. Injects `Ocp-Apim-Subscription-Key` from `IMF_API_PRIMARY_KEY` (with `IMF_API_SECONDARY_KEY` retried on 401/403 for live key rotation). Exposes `fetch_url` tool. |
