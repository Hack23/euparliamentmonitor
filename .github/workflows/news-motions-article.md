---
name: "News: EU Parliament Motions — Article"
description: Generates the EU Parliament motions English article once the paired analysis PR is merged. Stage D only. Reads committed analysis from main, optional bounded Stage-A top-up, single article PR.
strict: false
on:
  pull_request:
    types: [closed]
  ...
  workflow_dispatch:
    inputs:
      analysis_date:
        ...
        required: false
      article_type:
        description: 'Article type slug. Fixed to "motions" for this workflow.'
        required: false
        default: motions
  steps:
    - name: Check analysis PR trigger gate
      ...
      run: |
        ...
        if ! echo "$LABELS" | grep -q '"type:motions"'; then
          echo "PR missing 'type:motions' label — skipping article workflow"
          exit 1
        fi
...
safe-outputs:
  ...
  create-pull-request:
    title-prefix: "[news] "
    labels: [agentic-news, "type:motions"]
    draft: false
    expires: 14d
...
---
# 📰 EU Parliament Motions — Article Workflow
...
`news-motions-analysis.md` workflow.
...
3. The PR carries the `type:motions` label.
...
RUN_EPOCH=$(date -u +%s)
RUN_ID="motions-article-run$$-$RUN_EPOCH"
...
|-----------|-------|
| `ARTICLE_TYPE_SLUG` | `motions` |
| Family | Split: **article-only** (consumes analysis from `news-motions-analysis.md`) |
...
```bash
ARTICLE_HTML=$(ls -t "news/${ANALYSIS_DATE}-motions"*"-en.html" 2>/dev/null | head -1)
[ -n "$ARTICLE_HTML" ] && \
...
- `base: "main"`
- `head: "news/${ANALYSIS_DATE}-motions-${RUN_ID}"`
