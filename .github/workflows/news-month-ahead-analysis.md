---
name: "News: EU Parliament Month Ahead — Analysis"
description: Produces a single analysis-only PR for EU Parliament month ahead. Stages A + B + C only. Paired with news-month-ahead-article.md, which runs Stage D when this PR merges.
strict: false
on:
  schedule:
    - cron: "0 8 1 * *"
...
concurrency:
  group: "news-analysis-month-ahead"
...
safe-outputs:
  ...
  create-pull-request:
    ...
    labels: [agentic-analysis, analysis-data, "type:month-ahead"]
    draft: false
    expires: 14d
...
---
# 🔬 EU Parliament Month Ahead — Analysis-Only Workflow
...
(article drafting) happens in the paired `news-month-ahead-article.md` workflow
...
| Family | Split: **analysis-only** (paired with `news-month-ahead-article.md`) |
...
Stage D happens in the paired `news-month-ahead-article.md` workflow.
...
- `base: "main"`
- `head: "analysis/${TODAY}-month-ahead-${RUN_ID}"`
- `title: "[analysis] month-ahead — ${TODAY} (run ${RUN_ID})"`
