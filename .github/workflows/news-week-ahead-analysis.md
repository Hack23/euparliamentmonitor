---
name: "News: EU Parliament Week Ahead — Analysis"
description: Produces a single analysis-only PR for EU Parliament week ahead. Stages A + B + C only. Paired with news-week-ahead-article.md, which runs Stage D when this PR merges.
strict: false
on:
...
concurrency:
  group: "news-analysis-week-ahead"
...
safe-outputs:
  ...
  create-pull-request:
    ...
    labels: [agentic-analysis, analysis-data, "type:week-ahead"]
    draft: false
    expires: 14d
...
---
# 🔬 EU Parliament Week Ahead — Analysis-Only Workflow
...
(article drafting) happens in the paired `news-week-ahead-article.md` workflow
...
| Family | Split: **analysis-only** (paired with `news-week-ahead-article.md`) |
...
Stage D happens in the paired `news-week-ahead-article.md` workflow.
...
- `base: "main"`
- `head: "analysis/${TODAY}-week-ahead-${RUN_ID}"`
- `title: "[analysis] week-ahead — ${TODAY} (run ${RUN_ID})"`
