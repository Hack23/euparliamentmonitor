---
name: "News: EU Parliament Committee Reports — Analysis"
description: Produces a single analysis-only PR for EU Parliament committee reports. Stages A + B + C only. Paired with news-committee-reports-article.md, which runs Stage D when this PR merges.
strict: false
on:
...
concurrency:
  group: "news-analysis-committee-reports"
...
safe-outputs:
  ...
  create-pull-request:
    ...
    labels: [agentic-analysis, analysis-data, "type:committee-reports"]
    draft: false
    expires: 14d
...
---
# 🔬 EU Parliament Committee Reports — Analysis-Only Workflow
...
(article drafting) happens in the paired `news-committee-reports-article.md` workflow
...
| Family | Split: **analysis-only** (paired with `news-committee-reports-article.md`) |
...
Stage D happens in the paired `news-committee-reports-article.md` workflow.
...
- `base: "main"`
- `head: "analysis/${TODAY}-committee-reports-${RUN_ID}"`
- `title: "[analysis] committee-reports — ${TODAY} (run ${RUN_ID})"`
