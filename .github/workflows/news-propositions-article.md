---
name: "News: EU Parliament Propositions — Article"
description: Generates the EU Parliament propositions English article once the paired analysis PR is merged. Stage D only. Reads committed analysis from main, optional bounded Stage-A top-up, single article PR.
strict: false
on:
  pull_request:
    types: [closed]
  ...
  workflow_dispatch:
    inputs:
      analysis_date:
        ...
        required: false
      article_type:
        description: 'Article type slug. Fixed to "propositions" for this workflow.'
        required: false
  ...
  steps:
    - name: Check analysis PR trigger gate
      ...
      run: |
        ...
        if ! echo "$LABELS" | grep -q '"type:propositions"'; then
          echo "PR missing 'type:propositions' label — skipping article workflow"
          exit 1
        fi
...
concurrency:
  group: "news-article-propositions"
...
safe-outputs:
  ...
  create-pull-request:
    title-prefix: "[news] "
    labels: [agentic-news, "type:propositions"]
    draft: false
    expires: 14d
...
---
# 📰 EU Parliament Propositions — Article Workflow
...
`news-propositions-analysis.md` workflow.
...
3. The PR carries the `type:propositions` label.
...
RUN_EPOCH=$(date -u +%s)
RUN_ID="propositions-article-run$$-$RUN_EPOCH"
...
|-----------|-------|
| `ARTICLE_TYPE_SLUG` | `propositions` |
| Family | Split: **article-only** (consumes analysis from `news-propositions-analysis.md`) |
...
```bash
ARTICLE_HTML=$(ls -t "news/${ANALYSIS_DATE}-propositions"*"-en.html" 2>/dev/null | head -1)
[ -n "$ARTICLE_HTML" ] && \
...
- `base: "main"`
- `head: "news/${ANALYSIS_DATE}-propositions-${RUN_ID}"`
