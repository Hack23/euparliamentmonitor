...
- ✅ Monthly review analysis (evaluate legislative throughput, coalition evolution, institutional dynamics)
...
ARTICLE_HTML=$(ls -t "news/${TODAY}-month-in-review"*"-en.html" 2>/dev/null | head -1)
...
>
> **🛑 No-publish threshold**: If feed data yields fewer than 5 specific parliamentary items from the past month, create an analysis-only PR instead. See [SHARED_PROMPT_PATTERNS.md Minimum Publication Threshold](../prompts/SHARED_PROMPT_PATTERNS.md#-minimum-publication-threshold-no-publish-rule).
>
...
- **Minutes 40–45**: 📰 **ARTICLE GENERATION PHASE (5 MINUTES — 2 PASSES)** — **Analysis MUST be complete before generation starts.**
  - **Pass 1 (Minutes 40–43, ~3 min)**: Generate English article with deep political intelligence informed by completed analysis artifacts. Replace ALL `[AI_ANALYSIS_REQUIRED]` markers. Ensure ≥60% prose ratio.
...
# Month-in-Review Session Baseline — ${TODAY} (run ${RUN_ID})
...
- **title**: `Monthly review analysis checkpoint — ${TODAY} (run ${RUN_ID})` (the `[news] ` prefix is added automatically)
- **body**: `Baseline month-in-review analysis checkpoint for ${TODAY}. Engine crash-resilience PR — EP MCP data, deep political analysis artifacts, and the final article will be added automatically as the agent continues working.`
- **base**: `main`
- **head**: `news/month-in-review-${TODAY}` (use `TODAY` from the Date Context Establishment block above)
...
# 3. Direct EP API HTTP connectivity — test both meps and adopted-texts (diagnostic only)
...
# 6. Diagnosis (uses curl exit codes to distinguish DNS/connect/timeout failures)
...
4. **TIMEOUT FALLBACK**: When a feed endpoint times out (e.g. `get_procedures_feed`, `get_events_feed`), do **NOT** retry the same feed endpoint; use the corresponding non-feed endpoint with year filter instead:
   ...
   - `get_events_feed` timeout → call `european_parliament___get_events({ limit: 20 })` instead (v1.2.11: no date filtering for events)
...
> **📊 ADAPTIVE STRATEGY**: If health shows `Degraded`/`Sparse`/`Unavailable`, **enter DEGRADED MODE** immediately: widen initial timeframe for ALL feeds, use direct endpoint fallbacks for failed feeds, skip analytical tools that depend on upstream API calls, and focus on `get_all_generated_stats` for precomputed context.
...
1. **Use direct endpoint fallbacks** when feeds fail (see timeout fallback strategy below)
2. **Skip analytical tools that require live upstream API calls**: `detect_voting_anomalies`, `generate_political_landscape`, `early_warning_system` — record as `SKIPPED_DEGRADED_MODE` in manifest
3. **Focus on reliable data sources**: `get_all_generated_stats` (precomputed), `analyze_coalition_dynamics` (cached structural data), `get_adopted_texts` (year-based, always works)
4. **Still attempt ALL feed endpoints** — some feeds may work even when others don't
5. **Still write ALL analysis artifacts** — use precomputed stats and whatever data was collected
6. **Record degraded mode in manifest**: Set `"degradedMode": true`
...
european_parliament___get_speeches({ dateFrom: "<last-month>", dateTo: "<today>", limit: 20 })
```

> **🔴 VOTING EVIDENCE REQUIREMENT**: Monthly review articles MUST include actual voting record data when discussing this month's votes. Use `get_voting_records` and `get_meeting_decisions` to verify all coalition behaviour claims. If voting records are empty (EP publishes with delay), mark those claims as LOW confidence.

**CONDITIONAL analytical tools** (skip in DEGRADED MODE):

```javascript
// Voting anomalies — SKIP in DEGRADED MODE
european_parliament___detect_voting_anomalies({ dateFrom: "<last-month>", dateTo: "<today>" })
...
>
> **REJECTED title patterns for monthly review:**
> - ❌ Any title containing raw counts ("N adopted texts", "N procedures")
> - ❌ Any title starting with "Month in Review:" followed by dates
> - ❌ See [SHARED_PROMPT_PATTERNS.md Title Quality Rules](../prompts/SHARED_PROMPT_PATTERNS.md#-title-quality-rules)
>
> **REQUIRED: Write journalistic review headlines:**
> - ✅ `Record Q1 Output Reshapes EU Regulatory Landscape as Easter Recess Begins`
> - ✅ `Banking Union, Anti-Corruption, and Trade Defence Define March Sprint`
>
...
> AI_TITLE="Banking Union and Anti-Corruption Directive Cap Record March Sprint"
> AI_DESCRIPTION="European Parliament adopted 34 texts in concentrated March sessions, delivering Banking Union triple reform, first EU anti-corruption directive, and trade defence package"
> ```
