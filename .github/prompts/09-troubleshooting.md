...
| **Engine Failure — long-run Copilot crash with 429 reflection** — `copilot engine terminated unexpectedly` after a **long run (> 20 min)**.<br>Fingerprint: `exitCode: 1`, `hasOutput: true`, `stdout: 0B`, `stderr: ~259B`, all 3 process retries exhausted quickly, `awf-reflect: models fetch returned 429`, effective-tokens counter very high (> 50 M ET). Forensic examples: [run 25757187347](https://github.com/Hack23/euparliamentmonitor/actions/runs/25757187347) (`news-breaking`, 109.3 M ET) and [run 25781685994](https://github.com/Hack23/euparliamentmonitor/actions/runs/25781685994) (`news-propositions`, 112.1 M ET). | Treat high ET as the **symptom**, not the fix target. The riksdagsmonitor propositions workflow succeeds with the same model because its compiled workflow keeps continuation recovery enabled, uses cache-memory only, gives MCP backends a 180 s startup budget, and aligns MCP backend containers with Node 26. The unstable euparliamentmonitor drift was: `max-continuations: 1` disabled gh-aw autopilot continuation recovery, `repo-memory` compiled an extra `push_repo_memory` safe-output/write job not present in riksdagsmonitor, MCP backend startup budget was only 90 s, and shared MCP containers still used an older Node Alpine image while workflows run Node 26. | Restore the stable workflow posture instead of changing models: remove `max-continuations: 1` from article workflows, remove `tools.repo-memory` from news workflows (keep `cache-memory`), set `tools.startup-timeout: 180`, keep per-tool calls bounded at 180 s, use `node:26-alpine` for shared MCP backends, include Docker registry domains in `network.allowed`, then recompile with `gh aw compile --validate`. Re-run only after these configuration issues are fixed. |
