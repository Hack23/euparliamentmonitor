...
| `INTERNAL_ERROR` | MCP server internal failure — verify `european-parliament-mcp-server@1.3.4` installed; check DNS for `data.europarl.europa.eu`. |
...
| **`prefetch-ep-feeds.sh` exits 1 with `All N feed(s) failed — EP API unreachable`** — the pre-agent step exits 1 and the workflow run fails (the agent never starts). | EP Open Data API was unreachable at workflow start. The readiness probe (`curl --head` to `/meps?limit=1`) confirmed the API was down before any per-feed fetches were attempted. | Re-run the workflow once the EP API is back. The pre-agent step must succeed (exit 0) for the agent to start — this requires at least one feed to return usable data. If the EP API is chronically unavailable, consider adding `continue-on-error: true` to the pre-fetch step and relying on prior-run analysis data. Do NOT remove the readiness probe or reduce retry counts — 3 retries (5s/15s/45s) is the minimum viable backoff that handles transient EP API cold-start delays. Drift-guarded by `test/unit/prefetch-ep-feeds.test.js`. |
