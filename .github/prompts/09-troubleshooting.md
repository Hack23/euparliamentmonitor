...
| **`prefetch-ep-feeds.sh` exits 1 with `All N feed(s) failed — EP API unreachable`** — the pre-agent step exits 1 and marks all feeds as placeholders. | EP Open Data API was unreachable at workflow start. The readiness probe (`curl --head` to `/meps?limit=1`) confirmed the API was down before any per-feed fetches were attempted. | Re-run the workflow once the EP API is back. If the EP API is chronically unavailable, the agent will proceed with `{"status":"unavailable"}` placeholders and rely on prior-run analysis data. Do NOT remove the readiness probe or reduce retry counts — 3 retries (5s/15s/45s) is the minimum viable backoff that handles transient EP API cold-start delays. Drift-guarded by `test/unit/prefetch-ep-feeds.test.js`. |
