...
| **Engine Failure — rate-limit / high-ET crash** — `copilot engine terminated unexpectedly` (agent job) after a **long run (> 20 min)**.<br>Fingerprint: `exitCode: 1`, `hasOutput: true`, `stdout: 0B`, `stderr: ~259B`, all 3 retries exhausted quickly, `awf-reflect: models fetch returned 429`, effective-tokens counter very high (> 50 M ET). Forensic example: [run 25757187347](https://github.com/Hack23/euparliamentmonitor/actions/runs/25757187347) (`news-breaking`, 109.3 M ET, `claude-sonnet-4.6` at 9.0×). | The agent consumed too many effective tokens during the long Stage-B analysis pass, triggering Copilot API rate-limiting. The Copilot CLI exits with code 1 when the API returns 429; subsequent retry attempts also fail immediately because the rate limit is still active. The `awf-reflect` 429 on the models endpoint confirms the API is still rate-limited at post-exit diagnostic time. **Root cause is the model multiplier**: high-multiplier models (e.g. `claude-sonnet-4.6` = 9.0×) can exhaust the per-workflow ET budget 9× faster than the 1.0× reference model. | Switch `engine.model` to a lower-multiplier model (e.g. `claude-sonnet-4` = 1.0×). Recompile with `gh aw compile <workflow>`. The fix reduces effective-token consumption by the multiplier ratio (9× in this case) without sacrificing output quality for standard news-analysis workflows. Do NOT simply re-run — the retries will fail identically until the rate limit clears AND the model multiplier is reduced. |
