...
| `INTERNAL_ERROR` | MCP server internal failure — verify `european-parliament-mcp-server@1.3.3` installed; check DNS for `data.europarl.europa.eu`. |
...
| **Engine Failure — rate-limit / high-ET crash** — `copilot engine terminated unexpectedly` (agent job) after a **long run (> 20 min)**.<br>Fingerprint: `exitCode: 1`, `hasOutput: true`, `stdout: 0B`, `stderr: ~259B`, all 3 retries exhausted quickly, `awf-reflect: models fetch returned 429`, effective-tokens counter very high (> 50 M ET). Forensic examples: [run 25757187347](https://github.com/Hack23/euparliamentmonitor/actions/runs/25757187347) (`news-breaking`, 109.3 M ET, `claude-sonnet-4.6` at 9.0×); [run 25781685994](https://github.com/Hack23/euparliamentmonitor/actions/runs/25781685994) (`news-propositions`, 112.1 M ET, `claude-sonnet-4.6` at 9.0×). **This pattern affects all news workflows using high-multiplier models** — the same root cause was confirmed across news-breaking, news-propositions, and other workflows using `claude-sonnet-4.6` (9.0×) or `claude-opus-4.7` (27.0×). | The agent consumed too many effective tokens during the long Stage-B analysis pass, triggering Copilot API rate-limiting. The Copilot CLI exits with code 1 when the API returns 429; subsequent retry attempts also fail immediately because the rate limit is still active. The `awf-reflect` 429 on the models endpoint confirms the API is still rate-limited at post-exit diagnostic time. **Root cause is the model multiplier**: high-multiplier models (e.g. `claude-sonnet-4.6` = 9.0×, `claude-opus-4.7` = 27.0×) can exhaust the per-workflow ET budget 9–27× faster than the 1.0× reference model. euparliamentmonitor is particularly affected vs. simpler platforms (e.g. riksdagsmonitor) because the EP MCP server + IMF SDMX + World Bank data pipeline generates substantially larger context per run. | Switch `engine.model` to a lower-multiplier model: use `claude-sonnet-4` (1.0×) for standard news-analysis workflows; use `claude-opus-4` (5.0×) for complex long-horizon workflows (term-outlook, election-cycle). Recompile with `gh aw compile --validate`. The fix reduces effective-token consumption by the multiplier ratio (9× or 27× in these cases) without sacrificing output quality. Do NOT simply re-run — the retries will fail identically until the rate limit clears AND the model multiplier is reduced. |
