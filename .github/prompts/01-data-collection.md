...
## 4 · Economic Context — World Bank **or** IMF (Wave-2 OR-gate)
...
- Canonical indicator → article-type maps:
  - [`analysis/methodologies/worldbank-indicator-mapping.md`](../../analysis/methodologies/worldbank-indicator-mapping.md)
    (social / health / env / innovation)
  - [`analysis/methodologies/imf-indicator-mapping.md`](../../analysis/methodologies/imf-indicator-mapping.md)
    (macro / fiscal / trade / monetary + WEO forecasts)
- Connectivity probe: `source scripts/wb-mcp-probe.sh` after `scripts/mcp-setup.sh`.
- Either source satisfies the gate; the validator's evidence check accepts both.

Integration requirements:
1. Call `search-indicators` first to discover the best indicator.
2. Fetch ≥ 2 EU countries (Big Four or affected member states).
