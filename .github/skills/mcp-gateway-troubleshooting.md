...
### EU Parliament MCP Reliability (May 2026)

| Symptom | Likely Cause | Immediate Mitigation |
|---|---|---|
| `get_committee_documents_feed` returns 404/error envelope | Upstream feed instability in `european-parliament-mcp-server@1.3.14` | Fallback to `get_committee_documents({ limit: 20 })` and mark source as degraded |
...
```bash
# Local reliability matrix (token-safe output)
npm run mcp:probe

# CI schedule runner
# .github/workflows/mcp-probe-daily.yml
```
