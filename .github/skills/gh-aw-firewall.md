...
- Workflows using the IMF native SDMX REST client additionally allowlist `dataservices.imf.org` (primary economic source under Wave-3 editorial policy)
- Workflows using the World Bank MCP server additionally allowlist `api.worldbank.org` (non-economic domains only under Wave-3)
