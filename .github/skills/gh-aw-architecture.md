...
| `engine` | AI engine configuration. In this repo, Claude is selected via `engine: { id: copilot, model: claude-sonnet-4 }` (that is, `id` is `copilot` and the `model` field chooses the Claude variant). |
