...
| **7** | **[Artifact Catalog](artifact-catalog.md)** | Master map of every analysis markdown file produced under `analysis/daily/*/` — grouped by folder with methodology + template + depth floor + Mermaid type per artifact | 5 folder groups (intelligence · classification · risk-scoring · threat-assessment · documents) | **Step 2** — after reading the AI guide, agents consult this catalog to know *what* to produce |
| **8** | **[Per-Artifact Methodologies](per-artifact-methodologies.md)** | One section per artifact type with purpose, EP MCP inputs, required sections, mandatory Mermaid, depth floor, quality signals | ~24 artifacts across the 5 folder groups | **Step 2** — construction lookup; read the relevant section when writing each artifact |
