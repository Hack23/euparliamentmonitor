...
- The **canonical decay table** lives in [`forward-projection-methodology.md` §3 — WEP Decay Table](forward-projection-methodology.md#3-wep-decay-table), and is the single source of truth across the platform. This style guide deliberately does **not** duplicate the numbers — refer to the methodology so there is one row per horizon (`T+7d`, `T+30d`, `T+90d`, `T+12m`, `T+term-end`, `T+EP-election ±6m`) and one set of floor bands.
