...
- IMF (sole authoritative economic source — Wave-4) WEO / FM / IFS + World Bank WGI indicators (non-economic only — governance, social, demographics; **never** for economic context), Eurostat cross-country comparables
...
- IMF MCP (sole authoritative economic source — `imf-fetch-data` for WEO/FM/IFS/BOP/ER/PCPS/GFSR/EREO/FSI/GFS/DOT under Wave-4 policy) + World Bank MCP tools (non-economic only — governance WGI, social, health, education, environment, agriculture, innovation; **never** for economic context)
...
7. **Benchmark trend chart** — quantitative time-series Mermaid where IMF data exists (sole authoritative economic source under Wave-4 policy); World Bank cross-refs only for non-economic dimensions (never for economic context)
