---
title: "Easter recess continues. Parliament remains suspended until April 27. Run 187 — the ninth monitoring run in the Easter Recess series …"
...
pie title Coalition Alliance Signals by Size Similarity (Run 187)
    "ECR-PfE (0.96)" : 0.96
    "Renew-ECR (0.95)" : 0.95
    "Renew-PfE (0.92)" : 0.92
    "ESN-NI (0.90)" : 0.90
    "Greens-Left (0.87)" : 0.87
    "Other pairs" : 4.35
```
...
```mermaid
%%{init: {"theme":"dark","themeVariables":{"primaryColor":"#1565C0","primaryTextColor":"#ffffff","lineColor":"#90CAF9","xyChart":{"backgroundColor":"#1e1e1e","plotColorPalette":"#1565C0,#2E7D32,#FF9800"}}}}%%
xychart-beta
    title "Adopted Texts API Restoration Trajectory (2026)"
    x-axis ["Run 183", "Run 184", "Run 185", "Run 186", "Run 187"]
    y-axis "Texts Accessible" 0 --> 70
    line [30, 40, 45, 51, 61]
```
...
```mermaid
%%{init: {"theme":"dark","themeVariables":{"primaryColor":"#1565C0","primaryTextColor":"#ffffff","lineColor":"#90CAF9","secondaryColor":"#2E7D32","tertiaryColor":"#FF9800","mainBkg":"#1565C0"}}}%%
graph TB
    A[EU Parliament<br/>Democratic Function] --> B[External Trade Threats]
    A --> C[Institutional Accountability Gap]
    A --> D[Information Infrastructure]
    B --> E[US Tariff Escalation<br/>🟡 ELEVATED]
    B --> F[USTR Section 301<br/>Digital Regulation<br/>🔴 CRITICAL WATCH]
    B --> G[EU-China Backlash Risk<br/>🟡 NEW]
    C --> H[Recess Accountability Vacuum<br/>🟡 STRUCTURAL]
    C --> I[BRRD3 Bundesrat Risk<br/>🟡 MEDIUM]
    D --> J[Tier 2 Feed Outage<br/>🟡 OPERATIONAL]
    D --> K[Content Availability Gap<br/>🟡 INFORMATION]
```
...
graph TD
    ROOT[US Tariff Escalation Against EU] --> A[USTR Executive Action]
    ROOT --> B[Section 301 Investigation]
    A --> A1[Automotive sector tariffs 25%]
    A --> A2[Agricultural retaliatory tariffs]
    A --> A3[Tech/digital sector sanctions]
    B --> B1[EU AI Act investigation]
    B --> B2[EU DMA investigation]
    B --> B3[EU Data Act investigation]
    A1 --> X[EP INTA Emergency Session<br/>April 29-30 resolution required]
    A2 --> X
    B1 --> Y[EP Constitutional Crisis:<br/>Defending legislative sovereignty]
    B2 --> Y
    B3 --> Y
    X --> Z[Commission negotiation mandate<br/>EP consent required]
    Y --> W[EP resolution defending<br/>digital legislation legitimacy]
```
