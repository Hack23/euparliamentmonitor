...
```mermaid
graph LR
    A[Ukraine<br/>Accountability<br/>449+ seats] --> B[EPP Pivot<br/>Actor]
    C[DMA<br/>Enforcement<br/>EPP Split] --> B
    D[Agricultural<br/>Counter-Revolution<br/>349 seats] --> B
    B --> E[EP10 Identity:<br/>Geopolitics Firm<br/>Economics Contested]
```
...
```mermaid
graph TD
    EPP[EPP 183] -->|Drives| UKRAINE[Ukraine Accountability\nTA-0161/0154]
    SD[S&D 136] -->|Co-leads| UKRAINE
    RENEW[Renew 77] -->|Supports| UKRAINE
    GREENS[Greens/EFA 53] -->|Supports| UKRAINE
    LEFT[The Left 45] -->|Mixed| UKRAINE

    SD -->|Leads| DMA[DMA Enforcement\nTA-0160]
    GREENS -->|Strongest voice| DMA
    EPP -->|Fractured| DMA
    PFE[PfE 85] -->|Opposes| DMA
    ECR[ECR 81] -->|Opposes| DMA

    EPP -->|Alliance| AGRI[Livestock/Farming\nTA-0157]
    ECR -->|Alliance| AGRI
    PFE -->|Alliance| AGRI
    GREENS -->|Opposes| AGRI

    EPP -->|Mixed| ARMENIA[Armenia Resilience\nTA-0162]
    SD -->|Supports| ARMENIA
    RENEW -->|Supports| ARMENIA
    PFE -->|Opposes| ARMENIA
    ECR -->|Ambivalent| ARMENIA
```
...
```mermaid
graph LR
    IF[Issue Frame:<br/>Ukraine+DMA+Agriculture] --> DF[Driving Forces<br/>5 identified]
    IF --> RF[Restraining Forces<br/>4 identified]
    DF --> NP[Net Pressure:<br/>PRO-ACTIVISM]
    RF --> NP
    NP --> IP[Intervention Points]
    IP --> Council[Council adoption<br/>within 45 days]
    IP --> Commission[Commission DMA<br/>enforcement decision]
    IP --> EPP_Vote[EPP internal<br/>vote cohesion]
```
...
```mermaid
quadrantChart
    title Impact Matrix (Urgency vs Magnitude)
    x-axis Low Urgency --> High Urgency
    y-axis Low Impact --> High Impact
    quadrant-1 Strategic Priority
    quadrant-2 Act Now
    quadrant-3 Monitor
    quadrant-4 Important but Deferrable
    Ukraine Accountability: [0.85, 0.92]
    Claims Commission: [0.80, 0.88]
    DMA Enforcement: [0.70, 0.85]
    Armenia Resolution: [0.60, 0.55]
    Livestock Motion: [0.65, 0.68]
    Budget 2027: [0.55, 0.62]
```
...
```mermaid
graph LR
    EPP[EPP 183] --> |lead| UA[Ukraine<br/>Accountability]
    SD[S&D 136] --> |join| UA
    Renew[Renew 77] --> |join| UA
    Greens[Greens 53] --> |join| UA
    UA --> |449 seats 62.6%| Result[PASSED]
    EPP --> |split 50/50| DMA[DMA<br/>Enforcement]
    SD --> |join| DMA
    ECR[ECR 81] --> |oppose| DMA
    PfE[PfE 85] --> |oppose| DMA
    EPP --> |lead| Farm[Livestock<br/>Motion]
    ECR --> |join| Farm
    PfE --> |join| Farm
    Farm --> |349 seats| NearMaj[Near-Majority]
```
...
```mermaid
graph TB
    Tier1[TIER 1: PRIMARY ACTORS]
    EPP_G[EPP Group<br/>183 seats] --> Tier1
    SD_G[S&D Group<br/>136 seats] --> Tier1
    Commission[European<br/>Commission] --> Tier1
    
    Tier2[TIER 2: SECONDARY ACTORS]
    ECR_G[ECR Group<br/>81 seats] --> Tier2
    Renew_G[Renew Group<br/>77 seats] --> Tier2
    UkraineGov[Ukraine<br/>Government] --> Tier2
    BigTech[BigTech<br/>Apple/Google/Meta] --> Tier2
    
    Tier3[TIER 3: TERTIARY ACTORS]
    FarmerOrgs[EU Farmer<br/>Organizations] --> Tier3
    CSOs[Civil Society<br/>Organizations] --> Tier3
    G7[G7 Partners] --> Tier3
```
...
```mermaid
graph LR
    IMF[IMF WEO Spring 2026<br/>Source: IMF] --> Eurozone[Eurozone GDP<br/>1.2% growth 2026]
    IMF --> Ukraine[Ukraine Economy<br/>+4.5% projected with aid]
    IMF --> Global[Global Growth<br/>3.1% baseline]
    Eurozone --> |constrains| Budget[EU Budget 2027<br/>€1.2tn envelope]
    Budget --> Claims[Claims Commission<br/>Funding Source]
    Ukraine --> |magnifies| Accountability[Accountability<br/>Political Priority]
```
...
quadrantChart
    ...
    quadrant-3 Watch
    quadrant-4 Escalate
    R-A1 Ukraine Accountability Stall: [0.72, 0.88]
    R-A2 DMA Trade War: [0.55, 0.82]
    R-A3 Farm Vote Green Deal Rollback: [0.78, 0.65]
    R-A4 Coalition Fracture: [0.35, 0.75]
    R-A5 Claims Commission Delay: [0.60, 0.58]
    R-A6 EP Credibility Gap: [0.45, 0.50]
```
...
mindmap
    root((PESTLE<br/>Analysis))
        Political
            ...
            US-EU tensions
        Economic
            IMF Eurozone 1.2% growth
            Digital market contestation
            Agricultural sector stress
        Social
            Farmer protest legacy
            Refugee accountability demands
            Digital equity concerns
        Technological
            DMA platform regulation
            AI Act implementation
            Digital sovereignty debate
        Legal
            Claims commission precedent
            DMA enforcement mandates
            International humanitarian law
        Environmental
            Green Deal rollback pressure
            Livestock methane targets
...
```mermaid
timeline
    title EP Accountability Architecture Evolution
    2014 : Crimea annexation<br/>EP calls for Russia suspension
    2022 : Ukraine invasion<br/>EP adopts sanctions support resolutions
    2023 : Rule of law challenges<br/>EP-Commission dialogue on Article 7
    2024 : EP elections<br/>EPP consolidates, far-right gains
    2025 : EP10 begins<br/>New coalition dynamics established
    2026 : Claims Commission mandate<br/>DMA enforcement decision point
```
