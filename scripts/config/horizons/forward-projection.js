 ...
 */
export function getForwardStatementsHorizonDays(slug) {
    for (const cfg of Object.values(ARTICLE_HORIZONS)) {
        if (cfg.slug === slug)
            return cfg.forwardStatementsHorizonDays;
    }
    return 0;
}
/**
 ...
 */
export function getScenarioMaxHorizonMonths(slug) {
    for (const cfg of Object.values(ARTICLE_HORIZONS)) {
        if (cfg.slug === slug)
            return cfg.scenarioMaxHorizonMonths;
    }
    return 0;
}
/**
 ...
 */
export function getForwardStatementsHorizonMap() {
    const map = {};
    for (const cfg of Object.values(ARTICLE_HORIZONS)) {
