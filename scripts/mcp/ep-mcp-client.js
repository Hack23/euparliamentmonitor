 ...
 * (`european-parliament-mcp-server@1.3.1`). The news workflows, prompt
 ...
 *
 * Maps EP MCP Server v1.3.1 structured error codes and generic HTTP/network
...
function classifyToolError(message) {
    ...
    // EP MCP Server v1.3.1 structured error codes (matched case-insensitively)
 ...
 *    `european-parliament-mcp-server@1.3.1`) —
...
export class EuropeanParliamentMCPClient extends MCPConnection {
     ...
     * `european-parliament-mcp-server@1.3.1`.
     *
     * **Upstream date-filter contract (v1.2.14+, active on the pinned v1.3.1 server):** the upstream server
     ...
     *
     * No local post-filter is applied here. The repository is pinned to v1.3.1, so the
