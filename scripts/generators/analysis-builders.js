...
function deriveMistakesFromAnomalies(anomalies) {
  return anomalies
    ...
    .map((a) => ({
      ...
      alternative: `anomaly_type=${a.type}, severity=${a.severity ?? 'unknown'}`,
    }));
}
...
function buildVotingWhyText(patterns, polarization) {
  if (polarization && patterns.length > 0) {
    const fragmented =
      polarization.fragmentedGroups.length > 0
        ? ` fragmented_groups=${polarization.fragmentedGroups.join(',')}`
        : '';
    return `polarization=${polarization.assessment} index=${polarization.overallIndex} high_cohesion_groups=${polarization.highCohesionGroups.length} effective_blocs=${polarization.effectiveBlocs.toFixed(1)}${fragmented}`;
  }
  if (patterns.length > 0) {
    const highCount = patterns.filter((p) => p.cohesion > 0.8).length;
    return `${patterns.length} voting patterns, ${highCount} groups above 80% cohesion`;
  }
  return 'no voting pattern data available';
}
/**
...
function buildVotingPoliticalImpact(recordCount, adoptedCount, anomalyCount, intensity) {
  ...
  const base = `adopted=${adoptedCount} anomalies=${anomalyCount} total_votes=${recordCount}`;
  if (!intensity) return base;
  const marginPct = (intensity.averageMargin * 100).toFixed(0);
  return `${base} avg_margin=${marginPct}% close_votes=${intensity.closeVoteCount} decisive_votes=${intensity.decisiveVoteCount}`;
}
...
export function buildVotingAnalysis(dateFrom, dateTo, records, patterns, anomalies, questions) {
  ...
  return {
    ...
    impactAssessment: {
      political: buildVotingPoliticalImpact(
        realRecords.length,
        adoptedCount,
        realAnomalies.length,
        intensity
      ...
      economic:
        topTopics.length > 0 ? `topics=${topTopics.join('; ')}` : 'no specific topics identified',
      social:
        realQuestions.length > 0
          ? `${realQuestions.length} questions: ${realQuestions
              .slice(0, 2)
              .map((q) => q.topic)
              .join('; ')}`
          : 'no parliamentary questions filed',
      legal:
        realRecords.length > 0
          ? `adopted=${adoptedCount} rejected=${rejectedCount}`
          : 'no voting records',
      geopolitical: `votes=${realRecords.length} patterns=${realPatterns.length}`,
    },
    ...
    outlook:
      realAnomalies.length > 0
        ? `anomalies=${realAnomalies.length}${polarization && polarization.fragmentedGroups.length > 0 ? ` fragmented_groups=${polarization.fragmentedGroups.join(',')}` : ''}`
        : `anomalies=0 records=${realRecords.length}`,
  ...
  return {
    ...
    why:
      bottleneckProcedures.length > 0
        ? `bottleneck_procedures=${bottleneckProcedures.length}`
        : `events=${eventCount} active_procedures=${pipelineCount}`,
    stakeholderOutcomes: [
      ...(bottleneckProcedures.length > 0
        ? [
            {
    ...
    impactAssessment: {
      political: `plenary_events=${eventCount}`,
      economic:
        docCount > 0 ? `legislative_documents=${docCount}` : 'no economic legislation flagged',
      social:
        questionCount > 0
          ? `parliamentary_questions=${questionCount}`
          : 'no parliamentary questions',
      legal: pipelineCount > 0 ? `pipeline_procedures=${pipelineCount}` : 'no pipeline procedures',
      geopolitical: `events=${eventCount} committees=${committeeCount}`,
    },
    actionConsequences: [
      ...
      ...weekData.events.slice(0, 2).map((e) => ({
        ...
        consequence: e.description || `event_type=${e.type}`,
        severity: 'medium',
      })),
    ],
    mistakes: bottleneckProcedures.slice(0, 2).map((p) => ({
      ...
      alternative: `bottleneck_stage=${p.stage ?? 'committee'}`,
    })),
    outlook: `events=${eventCount} active_files=${pipelineCount} bottlenecks=${bottleneckProcedures.length}`,
...
function buildPropositionsWhy(healthScore, throughput) {
  ...
  return `pipeline_health=${pct}% throughput=${throughput} status=${pipelineHealthLabel(healthScore)}`;
}
/**
  ...
  return [
    {
      ...
      consequence: `health_score=${healthScore.toFixed(2)}`,
      severity: healthSeverity,
    },
    {
      action: `Throughput rate at ${throughput}`,
      consequence: `throughput=${throughput}`,
...
function buildPropositionsImpact(healthScore, throughput) {
  const pct = (healthScore * 100).toFixed(0);
  return {
    political: `pipeline_health=${pct}% throughput=${throughput}`,
    economic: `throughput=${throughput}`,
    social: `pipeline_health=${pct}%`,
    legal: throughput > 0 ? `procedures_active=${throughput}` : 'no active procedures',
    geopolitical: `pipeline_health=${pct}%`,
  };
}
/**
        ...
        ? [
            {
              ...
              alternative: `pipeline_health=${pct}%`,
            },
          ]
        : [],
    outlook: `pipeline_health=${pct}% throughput=${throughput} status=${pipelineHealthLabel(healthScore)}`,
...
export function buildCommitteeAnalysis(committees, date, lang = 'en') {
  ...
  return {
    ...
    stakeholderOutcomes: committees.slice(0, 4).map((c) => ({
      ...
      outcome: c.documents.length > 2 ? 'winner' : c.documents.length > 0 ? 'neutral' : 'loser',
      reason:
...
export function buildCommitteeMindmap(committees, _lang = 'en') {
  ...
  const domainNodes = activeCommittees.slice(0, 8).map((c, i) => {
    ...
    const colors = ['green', 'cyan', 'blue', 'purple', 'orange', 'yellow', 'magenta', 'red'];
    return {
  ...
  const connections = activeCommittees.slice(0, 3).flatMap((c, i) =>
...
function buildVotingMDStakeholders(
  ...
  return {
    citizen: {
      strengths:
        adoptedCount > 0 ? [{ text: s.votingAdopted(adoptedCount), severity: 'medium' }] : [],
      weaknesses:
        realAnomalies.length > 0
...
function buildBreakingMDStakeholders(
  ...
  return {
    citizen: {
      strengths:
        adoptedCount > 0 ? [{ text: s.breakingAdopted(adoptedCount), severity: 'medium' }] : [],
      weaknesses: anomalyRaw ? [{ text: s.breakingAnomalyWeakness, severity: 'high' }] : [],
      opportunities:
        procCount > 0 ? [{ text: s.breakingProceduresActive(procCount), severity: 'medium' }] : [],
    ...
    mep: {
      strengths:
        active.length > 0
          ...
          : [],
      weaknesses:
        inactiveCount > 0 ? [{ text: s.committeeInactive(inactiveCount), severity: 'medium' }] : [],
      opportunities: [{ text: s.committeeHearings, severity: 'medium' }],
      threats: [{ text: s.committeeCompetingPriorities, severity: 'medium' }],
    },
    ngo: {
      strengths:
        totalDocs > 0 ? [{ text: s.committeeDocuments(totalDocs), severity: 'medium' }] : [],
      weaknesses:
        inactiveCount > committees.length * 0.3
...
export function buildVotingMultiDimensionalSwot(records, patterns, anomalies, lang = 'en') {
  ...
  const political = makeDimension(
    ...
    lowCohesion.length > 0
      ? [{ text: s.votingLowCohesion(lowCohesion.length), severity: 'high' }]
      : [],
    [{ text: s.votingCrossParty, severity: 'medium' }],
    highSeverity.length > 0
      ? [{ text: s.votingHighSeverity(highSeverity.length), severity: 'high' }]
      : []
  );
...
export function buildProspectiveMultiDimensionalSwot(weekData, _label, lang = 'en') {
  ...
  const political = makeDimension(
    ...
    weekData.events.length > 0
      ...
      : [],
    bottlenecks > 0 ? [{ text: s.prospectiveBottlenecks(bottlenecks), severity: 'high' }] : [],
    [],
...
function buildBreakingMDDimensions(
  ...
  const political = makeDimension(
    'political',
    adoptedCount > 0 ? [{ text: s.breakingAdopted(adoptedCount), severity: 'high' }] : [],
    anomalyRaw ? [{ text: s.breakingAnomalyWeakness, severity: 'high' }] : [],
    coalitionRaw ? [{ text: s.breakingCoalitionOpportunity, severity: 'medium' }] : [],
    anomalyRaw ? [{ text: s.breakingAnomalyThreat, severity: 'high' }] : []
  );
  const economic = makeDimension(
    'economic',
    adoptedCount > 0 ? [{ text: s.breakingAdopted(adoptedCount), severity: 'medium' }] : [],
    procWeakness,
    procOpportunity,
    [{ text: s.breakingRapidEvents, severity: 'medium' }]
  );
  const social = makeDimension(
    'social',
    eventCount > 0 ? [{ text: s.breakingEvents(eventCount), severity: 'medium' }] : [],
    [],
    procOpportunity,
    [{ text: s.breakingRapidEvents, severity: 'medium' }]
  );
  ...
  const geopolitical = makeDimension(
    ...
    [],
    coalitionRaw ? [{ text: s.breakingCoalitionOpportunity, severity: 'medium' }] : [],
    anomalyRaw ? [{ text: s.breakingAnomalyThreat, severity: 'medium' }] : []
  );
...
export function buildPropositionsMultiDimensionalSwot(pipelineData, lang = 'en') {
  ...
  const political = makeDimension(
    'political',
    healthScore > 0.7 ? [{ text: s.propositionsHealthStrong(pct), severity: 'high' }] : [],
    healthScore < 0.5 ? [{ text: s.propositionsHealthWeak(pct), severity: 'high' }] : [],
    [{ text: s.propositionsPrioritisation, severity: 'medium' }],
    healthScore < 0.3 ? [{ text: s.propositionsCriticalCongestion, severity: 'high' }] : []
  );
  const economic = makeDimension(
    'economic',
    throughput >= 5 ? [{ text: s.propositionsThroughputGood(throughput), severity: 'medium' }] : [],
    throughput < 5 ? [{ text: s.propositionsThroughputLow(throughput), severity: 'medium' }] : [],
    [{ text: s.propositionsTrilogueAcceleration, severity: 'medium' }],
...
export function buildCommitteeMultiDimensionalSwot(committees, lang = 'en') {
  ...
  const economic = makeDimension(
    ...
    totalDocs > 0 ? [{ text: s.committeeDocuments(totalDocs), severity: 'medium' }] : [],
    inactiveCount > 0 ? [{ text: s.committeeInactive(inactiveCount), severity: 'medium' }] : [],
    committees.length > 0 ? [{ text: s.committeeHearings, severity: 'medium' }] : [],
    [{ text: s.committeeCompetingPriorities, severity: 'medium' }]
  );
  const social = makeDimension(
    'social',
  ...
  const legal = makeDimension(
    'legal',
    totalDocs > 0 ? [{ text: s.committeeDocuments(totalDocs), severity: 'high' }] : [],
    inactiveCount > committees.length * 0.3
      ? [{ text: s.committeeInactive(inactiveCount), severity: 'high' }]
      : [],
    committees.length > 0 ? [{ text: s.committeeHearings, severity: 'medium' }] : [],
