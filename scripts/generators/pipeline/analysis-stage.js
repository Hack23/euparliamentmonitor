...
import { assessLegislativeVelocityRisk, runAgentRiskAssessment, generateRiskAssessmentMarkdown, calculatePoliticalRiskScore, assessPoliticalCapitalAtRisk, buildQuantitativeSWOT, createScoredSWOTItem, createScoredOpportunityOrThreat, createRiskDriver, } from '../../utils/political-risk-assessment.js';
...
function buildPoliticalCapitalRiskMarkdown(_fetchedData, date) {
    ...
    const rows = capitalAssessments
        .map((a) => `| ${a.actor} | ${a.currentCapital} | ${a.capitalAtRisk.toFixed(1)} | ${a.timeHorizon} | ${a.riskDrivers.length} drivers |`)
        .join('\n');
    return (header +
...
function buildQuantitativeSwotMarkdown(fetchedData, date) {
    ...
    const strengths = [
        createScoredSWOTItem('Established democratic institutions and treaty-based procedures', 4, [
        ...
        createScoredSWOTItem(`Active legislative pipeline with ${procedures.length} procedures tracked`, Math.min(procedures.length / 5, 5), [
        ...
        ], 'medium', procedures.length > 5 ? 'improving' : 'stable'),
        createScoredSWOTItem(`Parliamentary oversight with ${votingRecords.length} recorded votes`, Math.min(votingRecords.length / 3, 5), [
        ...
        ], votingRecords.length > 0 ? 'medium' : 'low', 'stable'),
    ];
    const weaknesses = [
        createScoredSWOTItem('Complex multi-stakeholder decision-making across 27 member states', 3, [
        ...
        createScoredSWOTItem(`Limited real-time data availability (${mepUpdates.length} MEP updates)`, Math.max(2, 5 - mepUpdates.length / 10), [
    ...
    const opportunities = [
        createScoredOpportunityOrThreat(`${events.length} upcoming parliamentary events for policy advancement`, events.length > 3 ? 'likely' : 'possible', events.length > 5 ? 'major' : 'moderate', [
        ...
        ], 'medium', events.length > 3 ? 'improving' : 'stable'),
        createScoredOpportunityOrThreat(`Growing legislative output (${adoptedTexts.length} adopted texts)`, adoptedTexts.length > 2 ? 'likely' : 'possible', 'moderate', [
    ...
    const threats = [
        createScoredOpportunityOrThreat('External geopolitical pressures and policy disruption risks', 'possible', 'major', [
        ...
        createScoredOpportunityOrThreat(`Coalition instability risk (${coalitions.length} coalition data points)`, coalitions.length > 0 ? 'possible' : 'unlikely', 'moderate', [
        ...
        ], coalitions.length > 0 ? 'medium' : 'low', 'stable'),
    ];
...
function buildDocumentAnalysisMarkdown(fetchedData, date) {
    ...
    for (const feedKey of DOCUMENT_FEED_KEYS) {
        ...
        for (const raw of items) {
            ...
            if (outputBase) {
                ...
                const docContent = buildSingleDocumentAnalysis(item, docId, title, feedKey, date, fetchedData);
    ...
    const tableRows = documentEntries.length > 0
        ? documentEntries
            .map((d) => `| ${sanitizeCell(d.id)} | ${sanitizeCell(d.title.slice(0, 60))} | ${sanitizeCell(d.category)} | [${d.filename}](documents/${d.filename}) |`)
            .join('\n')
...
function buildSingleDocumentAnalysis(item, docId, title, category, date, fetchedData) {
    ...
    const docStrengths = [
        createScoredSWOTItem(`Document contributes to legislative transparency`, 3, [`Document ${docId} is publicly available`, `Category: ${category}`], 'medium', 'stable'),
    ];
    const docWeaknesses = [
        createScoredSWOTItem('Single document may have limited standalone impact', 2, ['Part of broader legislative context', 'Requires cross-referencing with related documents'], 'medium', 'stable'),
    ];
    const docOpportunities = [
        createScoredOpportunityOrThreat('Potential for policy influence and stakeholder engagement', 'possible', 'moderate', ['Legislative documents shape EU policy direction'], 'medium', 'stable'),
    ];
    const docThreats = [
        createScoredOpportunityOrThreat('Risk of legislative delay or amendment dilution', 'possible', 'moderate', ['Complex multi-party negotiation dynamics'], 'medium', 'stable'),
    ];
