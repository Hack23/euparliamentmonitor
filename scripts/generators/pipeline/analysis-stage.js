...
function buildLegislativeDisruptionMarkdown(fetchedData, date) {
    ...
    for (const raw of procedures.slice(0, 5)) {
        ...
        disruptions.push(`| ${id} | ${title.slice(0, 50)} | ${analysis.currentStage} | ${analysis.resilience} | ${disruptionCount} |`);
    }
    return (header +
...
function buildQuantitativeSwotMarkdown(fetchedData, date) {
    ...
    const procedures = safeArr(fetchedData, 'procedures');
    const strengths = [
        createScoredSWOTItem('Established democratic institutions and procedures', 4, ['Treaty-based institutional framework'], 'high', 'stable'),
        createScoredSWOTItem(`Active legislative pipeline with ${procedures.length} procedures`, Math.min(procedures.length / 5, 5), [`${procedures.length} procedures tracked`], 'medium', 'stable'),
    ];
    const weaknesses = [
        createScoredSWOTItem('Complex multi-stakeholder decision-making', 3, ['27 member states, 7 political groups'], 'high', 'stable'),
    ];
    const opportunities = [
        createScoredOpportunityOrThreat(`${events.length} upcoming parliamentary events`, 'likely', 'moderate', [`${events.length} events scheduled`], 'medium', 'stable'),
    ];
    const threats = [
        createScoredOpportunityOrThreat('External geopolitical pressures', 'possible', 'major', ['Global political dynamics'], 'medium', 'stable'),
    ];
...
function buildLegislativeVelocityRiskMarkdown(fetchedData, date) {
    ...
    const riskRows = velocityRisks.length > 0
        ? velocityRisks
            .slice(0, 10)
            .map((r) => `| ${r.procedureId} | ${r.title.slice(0, 40)} | ${r.currentStage} | ${r.daysInCurrentStage}d / ${r.expectedDaysForStage}d | ${r.velocityRisk.riskScore.toFixed(2)} | ${r.velocityRisk.riskLevel} |`)
            .join('\n')
        : EMPTY_TABLE_ROW_6;
    return (header +
        `# Legislative Velocity Risk
...
function buildAgentRiskWorkflowMarkdown(fetchedData, date) {
    ...
    const workflow = runAgentRiskAssessment(`ASSESS-${date}`, date, 'week-ahead', identifiedRisks, riskDrivers, ['Monitor legislative velocity indicators', 'Track coalition voting patterns']);
