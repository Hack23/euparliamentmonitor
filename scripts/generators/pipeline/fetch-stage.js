...
function parseFeedResult(result) {
  ...
  const candidates = [parsed['data'], parsed['feed'], parsed['entries'], parsed['items'], parsed];
...
function parseFeedEnvelope(result) {
  ...
  for (const candidate of candidates) {
    if (Array.isArray(candidate)) return { items: candidate, total };
  }
  return { items: [], total };
}
/**
