...
import { parsePlenarySessions, parseCommitteeMeetings, parseLegislativeDocuments, parseLegislativePipeline, parseParliamentaryQuestions, buildWeekAheadContent, buildKeywords, PLACEHOLDER_EVENTS, } from './week-ahead-content.js';
...
export { parsePlenarySessions, parseCommitteeMeetings, parseLegislativeDocuments, parseLegislativePipeline, parseParliamentaryQuestions, buildWeekAheadContent, buildKeywords, PLACEHOLDER_EVENTS, };
 ...
 */
async function fetchMEPInfluence(mepId) {
    if (!mepId || !mcpClient) {
        return '';
    }
    try {
...
async function generateCommitteeReports() {
    ...
    try {
        ...
        const committeeDataRaw = await Promise.all(FEATURED_COMMITTEES.map((abbr) => fetchCommitteeData(abbr).catch((error) => {
            const message = error instanceof Error ? error.message : String(error);
 ...
 */
async function fetchVotingRecords(dateFromStr, dateStr) {
    if (!mcpClient)
        return [];
    try {
 ...
 */
async function fetchVotingPatterns(dateFromStr, dateStr) {
    if (!mcpClient)
        return [];
    try {
 ...
 */
async function fetchMotionsAnomalies(dateFromStr, dateStr) {
    if (!mcpClient)
        return [];
    try {
 ...
 */
async function fetchParliamentaryQuestions(dateFromStr, dateStr) {
    if (!mcpClient)
        return [];
    try {
