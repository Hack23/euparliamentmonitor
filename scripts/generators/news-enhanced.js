 ...
 * The AI agent (Opus 4.7) must analyse the content and produce this.
 */
...
async function maybeRunAnalysis(date, client) {
    ...
    console.log('');
    console.log(`🔬 Analysis discovery complete: ${completedCount} files found, ${skippedCount} skipped, ${failedCount} issues (of ${totalMethods})`);
    ...
    if (failedCount > 0) {
        ...
        throw new Error(`Analysis incomplete: ${failedCount} of ${totalMethods} discovered analysis files had issues (${failedNames}). ` +
            'Article generation requires analysis artifacts to exist.');
    }
    if (ctx.completedMethods.length === 0) {
        throw new Error(`Analysis produced no discovered files (${failedCount} issues). ` +
 ...
 * The AI agent (Opus 4.7) passes these after analysing the content.
...
export function computeDedupSuffix(articleTypes, analysisDir) {
    ...
    const isValidSuffix = rawSuffix === '' || RUN_ID_SUFFIX.test(rawSuffix) || DEDUP_SUFFIX.test(rawSuffix);
...
async function main() {
    ...
    try {
        ...
        for (const articleType of articleTypes) {
            ...
            results.push(await generateArticleForStrategy(strategy, client, languages, outputOptions, stats, dedupSuffix, analysisDir, analysisFiles));
        }
        console.log('');
