...
import { ALL_LANGUAGES, LANGUAGE_PRESETS, WEEK_AHEAD_TITLES, MOTIONS_TITLES, getLocalizedString, isSupportedLanguage, } from '../constants/languages.js';
 ...
 */
async function fetchVotingAnomalies(dateFromStr, dateStr) {
    if (!mcpClient)
        return [];
    try {
 ...
 */
function getMotionsFallbackData(dateStr, dateFromStr) {
    return {
        votingRecords: [
            {
 ...
 */
function generateMotionsContent(dateFromStr, dateStr, votingRecords, votingPatterns, anomalies, questions) {
    return `
    <div class="article-content">
      <section class="lede">
