 ...
 */
export function computeLegacyPrefix(date, slug, langCode) {
 ...
 */
function stripTruncatedReaderLabel(description, langCode) {
 ...
 */
function findTruncatedReaderLabelCut(text, langCode) {
 ...
 */
export function hasTruncatedReaderLabel(date, slug, lang, description) {
    if (!description)
        return false;
    const langCode = (lang || 'en').toLowerCase();
    const prefix = computeLegacyPrefix(date, slug, langCode);
 ...
 */
export function stripDuplicatedLegacyPrefix(trimmedDescription, prefix) {
 ...
 */
export function stripRedundantDateLabel(description, langCode, date) {
    if (!description)
        return description;
    const labels = getLocalizedString(SEO_CONTEXT_LABELS, langCode);
 ...
 */
export function capDescriptionLength(text) {
    if (text.length <= 180)
        return text;
 ...
 */
export function descriptionHasLeakyToken(description) {
    if (!description)
        return false;
 ...
 */
export function shouldBackfillDescription(description, descriptions) {
    return (!description ||
 ...
 */
export function buildBackfillManifest(runId, title, description, includeDescription) {
    return {
        runId,
        title,
 ...
 */
export function readArticleHtml(filepath) {
    try {
...
//# sourceMappingURL=backfill-strip.js.map