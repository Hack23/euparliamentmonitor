...
export function generateArticleHTML(options) {
    ...
    const safeSriAttrs = stylesHash && SRI_HASH_PATTERN.test(stylesHash)
        ...
        : '';
    // Compute SHA-256 hash of the JSON-LD inline script for CSP allowlisting
    const jsonLdInlineContent = `\n  ${jsonLd}\n  `;
    const jsonLdHash = createHash('sha256').update(jsonLdInlineContent, 'utf8').digest('base64');
    return `<!DOCTYPE html>
...
<head>
  ...
  <meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'sha256-${jsonLdHash}'; style-src 'self' 'unsafe-inline'; img-src 'self' https: data:; font-src 'self'; connect-src 'none'; frame-src 'none'; base-uri 'self'; form-action 'none'">
