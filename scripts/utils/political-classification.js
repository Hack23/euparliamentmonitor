...
function inferActorType(name) {
    ...
    if (MEMBER_STATE_KEYWORDS.some((kw) => lower.includes(kw)))
        return 'member_state';
    if (CIVIL_SOCIETY_KEYWORDS.some((kw) => lower.includes(kw)))
        return 'civil_society';
    if (MEDIA_KEYWORDS.some((kw) => lower.includes(kw)))
        return 'media';
    if (INDUSTRY_KEYWORDS.some((kw) => lower.includes(kw)))
        return 'industry';
...
export function analyzePoliticalForces(data) {
    ...
    const externalEvents = safeArray(data.events).filter((e) => EXTERNAL_KEYWORDS.some((kw) => asStr(e.title).toLowerCase().includes(kw) || asStr(e.description).toLowerCase().includes(kw))).length;
 ...
 * @example
 * ```ts
 * writeAnalysisFile('./analysis/2026-03-26/plenary-session/classification/significance-classification.md', fm, body);
 * ```
 */
