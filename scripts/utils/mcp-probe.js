...
import { EP_MCP_TOOLS, closeEPMCPClient, getEPMCPClient, } from '../mcp/ep-mcp-client.js';
import { IMF_MCP_TOOLS, closeIMFMCPClient, getIMFMCPClient, } from '../mcp/imf-mcp-client.js';
import { WORLD_BANK_MCP_TOOLS, closeWBMCPClient, getWBMCPClient, } from '../mcp/wb-mcp-client.js';
...
async function probeEPTool(client, tool) {
...
async function probeIMFTool(client, tool) {
    switch (tool) {
...
async function probeWBTool(client, tool) {
...
async function runProbe() {
    ...
    const probeOne = async (server, tool, invoker) => {
        const startedAt = Date.now();
        try {
            const result = await callWithTimeoutRetry(invoker, TOOL_TIMEOUT_MS, TOOL_TIMEOUT_RETRIES, tool);
 ...
 */
export function renderHealthMatrix(report) {
    ...
    lines.push(`Summary: total=${String(report.summary.total)} green=${String(report.summary.green)} yellow=${String(report.summary.yellow)} red=${String(report.summary.red)}`);
    return lines.join('\n');
}
/* c8 ignore start */
async function main() {
