...
function resolveOrRebuildMaps(parsed, articles, empty) {
  ...
  return {
    ...
    policyDomains: validDomains ? sanitizeMap(parsed.policyDomains) : empty.policyDomains,
    procedures: validProcedures ? sanitizeMap(parsed.procedures) : empty.procedures,
    rebuilt: false,
  };
}
...
export function buildRelatedArticlesHTML(relatedArticles, crossRefs, trends, lang) {
  ...
  const listItems = crossRefs
    .map((ref) => {
      ...
      const label = strings.relationships[ref.relationship] ?? strings.relatedArticle;
      if (article) {
