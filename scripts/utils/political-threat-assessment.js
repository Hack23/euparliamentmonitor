...
export function assessPoliticalThreats(data) {
    ...
    const overallThreatLevel = aggregateImpactLevels(strideCategories.map((c) => c.threatLevel));
...
function buildConsequenceTrees(data) {
    ...
    for (const anomaly of anomalies) {
        ...
        if (trees.length >= 3)
            break; // Limit to top 3 trees for output manageability
    }
...
export function generateThreatAssessmentMarkdown(assessment) {
    ...
    for (const cat of assessment.strideCategories) {
        ...
        lines.push(`### ${STRIDE_LABELS[cat.category]}`, `**Threat Level**: ${emoji} ${cat.threatLevel.charAt(0).toUpperCase() + cat.threatLevel.slice(1)}`, '', cat.analysis, '');
