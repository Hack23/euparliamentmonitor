 ...
 */
function buildDimensionResult(category, threatScore, evidence, defaultEvidence, lowAnalysis, elevatedAnalysis) {
...
function scanAnomaliesForShift(anomalies, evidence) {
    if (anomalies.length === 0)
        return 1;
    evidence.push(`${anomalies.length} voting anomal${anomalies.length === 1 ? 'y' : 'ies'} detected`);
    return 2;
}
/**
...
function scanCommitteesForTransparency(committees, evidence) {
    ...
    for (const committee of committees) {
        ...
        if (asNum(rec['meetingCount'], 0) === 0) {
            evidence.push(`Committee with no recorded meetings: ${asStr(rec['committeeId'] ?? rec['name'])}`);
            maxScore = Math.max(maxScore, 2);
        }
    }
    return maxScore;
...
function checkMEPInfluenceConcentration(mepInfluence, evidence) {
    ...
    for (const mep of mepInfluence) {
        ...
        if (rec)
            scores.push(asNum(rec['overallScore'], 0));
    }
    if (scores.length === 0)
        return 1;
    const max = scores.reduce((currentMax, value) => (value > currentMax ? value : currentMax), scores[0] ?? 0);
    const avg = scores.reduce((s, v) => s + v, 0) / scores.length;
    if (max > avg * 2.5) {
        evidence.push(`Highly concentrated MEP influence detected — top score ${max.toFixed(0)} vs avg ${avg.toFixed(0)}`);
        return 2;
    }
    return 1;
}
...
function checkMEPTurnover(feedData, evidence) {
    ...
    if (incoming.length > 20) {
        evidence.push(`High MEP turnover: ${incoming.length} incoming MEPs — institutional continuity risk`);
        return 2;
    }
    return 1;
}
...
function scanAbstentionAnomalies(anomalies, evidence) {
    ...
    if (abstentionAnomalies.length > 0) {
        evidence.push(`${abstentionAnomalies.length} abstention anomal${abstentionAnomalies.length === 1 ? 'y' : 'ies'} — potential procedural obstruction`);
        return 2;
    }
    return 1;
}
...
function countWeakCohesionCoalitions(coalitions, evidence) {
    ...
    for (const coalition of coalitions) {
        const rec = toRecord(coalition);
        if (rec && asNum(rec['cohesionScore'], 1) < 0.6)
            weakCount++;
    }
    if (weakCount >= 3) {
        evidence.push(`${weakCount} coalitions with critically low cohesion (<60%) — systemic erosion signal`);
        return 3;
    }
    if (weakCount >= 1) {
...
function scanCriticalAnomaliesForErosion(anomalies, evidence) {
    ...
    if (critical.length > 0) {
        evidence.push(`${critical.length} high-significance voting anomal${critical.length === 1 ? 'y' : 'ies'} — norm deviation signals`);
        return 2;
    }
    return 1;
}
...
function assessShiftThreats(data) {
    ...
    return buildDimensionResult('shift', threatScore, evidence, 'No coalition shift signals detected in available data', 'Coalition stability appears maintained. No significant realignment signals.', (level) => `Coalition dynamics show ${level}-level shift risk. Monitor cohesion trends closely.`);
}
/**
...
function assessTransparencyThreats(data) {
    ...
    if (questions.length > 0) {
        evidence.push(`${questions.length} parliamentary question${questions.length === 1 ? '' : 's'} submitted — active oversight`);
    }
    return buildDimensionResult('transparency', threatScore, evidence, 'Procedural transparency within normal parameters', 'Procedural transparency appears adequate. Committee activity within normal parameters.', (level) => `Transparency concerns at ${level} level. Review committee meeting records and public documentation.`);
}
/**
...
function assessReversalThreats(data) {
    ...
    return buildDimensionResult('reversal', threatScore, evidence, 'No significant policy reversal signals detected', 'Legislative trajectory appears stable. No major reversal signals.', (level) => `Policy reversal risk at ${level} level. Monitor stalled procedures and conciliation stages.`);
}
/**
...
function assessInstitutionalThreats(data) {
    ...
    return buildDimensionResult('institutional', threatScore, evidence, 'No institutional threat signals detected', 'Institutional balance appears maintained. Power distribution within normal parameters.', (level) => `Institutional threats at ${level} level. Monitor concentration of influence and procedural manipulation signals.`);
}
/**
...
function assessDelayThreats(data) {
    ...
    return buildDimensionResult('delay', threatScore, evidence, 'No significant legislative delay signals detected', 'Legislative pace within normal parameters. No obstruction signals.', (level) => `Delay threats at ${level} level. Monitor stalled procedures and abstention patterns.`);
}
/**
...
function assessErosionThreats(data) {
    ...
    return buildDimensionResult('erosion', threatScore, evidence, 'Democratic norms appear stable. No systematic erosion signals.', 'Democratic norms appear stable. Institutional processes functioning within expected parameters.', (level) => `Democratic erosion signals at ${level} level. Monitor norm deviations and coalition fragmentation patterns.`);
}
...
export function assessPoliticalThreats(data) {
    ...
    const keyFindings = threatDimensions
    ...
    for (const profile of actorProfiles) {
        if (profile.overallThreatLevel === 'high' || profile.overallThreatLevel === 'critical') {
            keyFindings.push(`High-threat actor: ${profile.actor} (${profile.actorType}) — CMO: ${profile.capability}/${profile.motivation}/${profile.opportunity}`);
        }
    }
    ...
    for (const cat of threatDimensions) {
        if (cat.threatLevel === 'critical' || cat.threatLevel === 'high') {
            recommendations.push(`Monitor ${DIMENSION_LABELS[cat.category].toLowerCase()} — ${cat.evidence[0] ?? 'elevated threat level'}`);
        }
    }
    for (const profile of actorProfiles) {
        if (profile.overallThreatLevel === 'critical' || profile.overallThreatLevel === 'high') {
            recommendations.push(`Track ${profile.actor} (${profile.actorType}) — ${profile.threatCategories.join(', ')} threat categories`);
        }
    }
...
function buildCoalitionProfile(rec) {
    ...
    if (riskLevel === 'high')
        threatCategories.push('erosion');
    if (trend === 'weakening')
        threatCategories.push('delay');
    return {
...
export function buildActorThreatProfiles(data) {
    ...
    return profiles.sort((a, b) => IMPACT_WEIGHTS[b.overallThreatLevel] - IMPACT_WEIGHTS[a.overallThreatLevel]);
}
/**
...
export function buildConsequenceTree(action, data) {
    ...
    if (anomalies.length > 0) {
        amplifyingFactors.push(`${anomalies.length} existing voting anomal${anomalies.length === 1 ? 'y' : 'ies'} amplify instability`);
    }
    ...
    return {
        ...
        mitigatingFactors,
        amplifyingFactors: amplifyingFactors.length > 0
            ? amplifyingFactors
...
function stageThreatCategory(stage) {
    ...
    if (stage === 'conciliation')
        return 'reversal';
    if (stage === 'plenary_first_reading' || stage === 'plenary_second_reading')
        return 'shift';
    return 'delay';
}
/**
...
export function analyzeLegislativeDisruption(procedure, data) {
    ...
    const disruptionPoints = ALL_LEGISLATIVE_STAGES.map((stage) => buildDisruptionPoint(stage, baseRisk, coalitionRisk));
...
function buildDisruptionTableMarkdown(analysis) {
    ...
    for (const point of analysis.disruptionPoints) {
        const riskLevel = point.likelihood > 0.4 ? '🔴 High' : point.likelihood > 0.25 ? '⚠️ Medium' : '🟢 Low';
...
export function generateThreatAssessmentMarkdown(assessment) {
    ...
    for (const cat of safeAssessment.threatDimensions) {
        ...
        lines.push(`### ${DIMENSION_LABELS[cat.category]}`, `**Threat Level**: ${emoji} ${cat.threatLevel.charAt(0).toUpperCase() + cat.threatLevel.slice(1)}`, '', sanitizeMarkdownText(cat.analysis), '');
 ...
 */
function describeThreatCorrelation(dimA, dimB, score) {
    if (score > 0.6)
    ...
    if (score > 0)
        return `${dimA} and ${dimB} have weak positive correlation`;
    if (score < -0.3)
 ...
 */
export function computeThreatCorrelationMatrix(dimensions) {
 ...
 */
export function detectEmergingThreats(currentThreats, baselineThreats, detectionDate) {
