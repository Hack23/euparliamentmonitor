 ...
 */
const COALITION_ANALYSIS_DEPTH_KEYWORDS = [
    'inter-group',
    'cross-party',
    'grand coalition',
    'qualified majority',
...
const TEMPORAL_CURRENT_KEYWORDS = [
    'currently',
    'present',
    'today',
    'ongoing',
...
const TEMPORAL_FORWARD_KEYWORDS = [
    'forecast',
    'projection',
    'expected',
    'anticipated',
 ...
 */
function computeCoalitionAnalysisScore(text) {
    const present = COALITION_ANALYSIS_DEPTH_KEYWORDS.filter((kw) => containsAnyKeyword(text, [kw])).length;
 ...
 */
export function scoreTemporalCoverage(htmlOrText, preExtracted = false) {
    ...
    const presentCount = [pastContextPresent, currentStatePresent, forwardOutlookPresent].filter(Boolean).length;
 ...
 */
export function scoreCrossReferenceDensity(html) {
    const htmlNoScripts = stripScriptBlocks(html);
    // Count unique EP document IDs (reuses EP_DOC_PATTERNS)
    const epDocSet = new Set();
    for (const pattern of EP_DOC_PATTERNS) {
        pattern.lastIndex = 0;
        const hits = htmlNoScripts.match(pattern);
        if (hits) {
    ...
    const totalReferenceSet = new Set([...epDocSet, ...taMatches, ...procMatches]);
...
function addAnalysisDepthRecommendations(depth, recs) {
    if (!depth.politicalContextPresent) {
        recs.push('Add political context: discuss coalitions, vote margins, and current parliamentary dynamics');
    }
    if (!depth.coalitionDynamicsAnalyzed) {
        recs.push('Analyse coalition dynamics — name specific inter-group alliances (e.g. EPP+Renew, S&D+Greens) and their vote margins');
    }
    if (!depth.historicalContextProvided) {
        recs.push('Provide historical context: compare to previous parliamentary terms, cite precedents or earlier decisions');
    }
    if (!depth.evidenceBasedConclusions) {
        recs.push('Support conclusions with specific data: cite vote counts, attendance figures, or referenced EP documents');
    }
    if (!depth.scenarioPlanning) {
        recs.push('Include forward-looking analysis: describe at least two scenarios (e.g. "if the amendment passes" vs "if rejected")');
    }
    if (!depth.confidenceLevelsIndicated) {
        recs.push('State confidence levels explicitly (e.g. "likely", "uncertain") to reflect analytical rigour');
    }
    if (depth.coalitionAnalysisScore !== undefined && depth.coalitionAnalysisScore < 30) {
        recs.push('Deepen coalition analysis: add rapporteur names, qualified-majority thresholds, or trilogue references');
    }
}
/**
