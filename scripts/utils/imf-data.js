 ...
 */
export const IMF_EU_COUNTRY_CODES = {
    AT: 'AUT',
    BE: 'BEL',
    BG: 'BGR',
    HR: 'HRV',
 ...
 */
export const IMF_COUNTRY_CODE_OVERRIDES = {
 ...
 */
export const IMF_AGGREGATE_LABELS = {
    EU: 'European Union',
    EA: 'Euro Area',
 ...
 */
export const IMF_INDICATOR_SDMX_CODES = Array.from(new Set(Object.values(IMF_POLICY_INDICATORS).map((m) => m.indicator)));
// ─── Country Code Lookup ───────────────────────────────────────────────────────
/**
 ...
 */
export function getIMFCountryCode(iso2Code) {
    if (!iso2Code)
        return null;
 ...
 */
export function isIMFEUMemberState(iso2Code) {
    if (!iso2Code)
        return false;
 ...
 * @internal
 */
function observationIsForecast(obsAttributes, obsStatusAttributeIndex, forecastCodeIndex) {
    if (!obsAttributes)
        return false;
    if (obsStatusAttributeIndex < 0)
        return false;
 ...
 * @internal
 */
function deserialiseSDMXPayload(raw) {
    if (!raw)
        return null;
 ...
 * @internal
 */
function buildDecodingContext(structure) {
    ...
    const forecastCodeIndex = obsStatusAttrIndex >= 0
        ? (obsAttributes[obsStatusAttrIndex]?.values ?? []).findIndex((v) => v.id.toUpperCase() === 'F')
        : -1;
 ...
 * @internal
 */
function coerceObservationValue(rawValue) {
 ...
 * @internal
 */
function decodeObservations(seriesObservations, ctx) {
    const out = [];
 ...
 */
export function getMostRecentObservation(observations) {
 ...
 */
export function getForecastPoints(observations, vintage) {
    return observations
...
//# sourceMappingURL=imf-data.js.map