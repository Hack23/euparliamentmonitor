...
function deriveRiskLevel(score) {
    ...
    if (score <= RISK_LEVEL_THRESHOLDS.MEDIUM_MAX)
        return 'medium';
    if (score <= RISK_LEVEL_THRESHOLDS.HIGH_MAX)
        return 'high';
    return 'critical';
}
/**
 ...
 */
export function calculatePoliticalRiskScore(likelihood, impact, riskId = 'RISK-AUTO', description = '', evidence = [], mitigatingFactors = [], confidence = 'medium') {
 ...
 */
export function assessPoliticalCapitalAtRisk(actor, actorType, currentCapital, riskDrivers, timeHorizon = 'quarter', confidenceInterval = 95) {
...
export function buildQuantitativeSWOT(title, strengths, weaknesses, opportunities, threats) {
    ...
    const strategicPositionScore = totalScore > 0 ? round2(clamp((positiveScore / totalScore) * 10, 0, 10)) : 5;
    ...
    const overallAssessment = strategicPositionScore >= 7
...
export function assessLegislativeVelocityRisk(procedures) {
    ...
    for (const raw of procedures) {
        ...
        const currentStage = isLegislativeStage(stageRaw)
            ? stageRaw
        ...
        const velocityRisk = calculatePoliticalRiskScore(likelihood, impact, `VEL-${procedureId}`, `Legislative velocity risk: ${title} has been in ${currentStage} stage for ${daysInCurrentStage} days (expected: ${expectedDays})`, [
            `Stage: ${currentStage}`,
            `Days in stage: ${daysInCurrentStage}`,
            `Expected: ${expectedDays} days`,
        ], velocityRatio < 1 ? ['Procedure is within expected timeline'] : [], velocityRatio < 0.5 ? 'high' : 'medium');
 ...
 */
export function runAgentRiskAssessment(assessmentId, date, articleType, identifiedRisks, riskDrivers, mitigations) {
    // Step 1: Identify
    ...
    // Step 4: Treat
    const treatStep = { type: 'treat', mitigations };
...
export function generateRiskAssessmentMarkdown(assessment) {
    const { assessmentId, date, articleType, overallRiskProfile, steps, recommendations } = assessment;
    ...
    const risksSection = identifyStep?.type === 'identify' ? buildRisksMarkdown(identifyStep.risks) : '';
    const evaluateStep = steps.find((s) => s.type === 'evaluate');
    const evaluateSection = evaluateStep?.type === 'evaluate' ? buildEvaluateMarkdown(evaluateStep.matrix) : '';
 ...
 */
export function generatePoliticalRiskSummary(date, topRisks, capitalAtRisk, quantitativeSwot, legislativeVelocityRisks) {
...
function synthesiseOverallRisk(risks, assessmentId, date) {
    ...
    const dominantConfidence = confidenceCounts.high >= confidenceCounts.medium &&
...
function buildRisksMarkdown(risks) {
    ...
    const lines = risks.map((r) => {
        ...
        const mitigations = r.mitigatingFactors.length > 0
 ...
 */
export function createScoredSWOTItem(description, score, evidence = [], confidence = 'medium', trend = 'stable') {
    return {
        description,
 ...
 * @returns ScoredSWOTItem
 */
export function createScoredOpportunityOrThreat(description, likelihood, impact, evidence = [], confidence = 'medium', trend = 'stable') {
