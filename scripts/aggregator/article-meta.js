...
import { extractExecutiveLead, extractLeadParagraph, trimToLeadSentence, } from './lead-extractor.js';
import { buildKeyTakeaways as _buildKeyTakeaways, jaccardSimilarity, extractStrongBullets, harvestCandidates, MAX_TAKEAWAYS, MIN_TAKEAWAYS, } from './key-takeaways.js';
...
const DATE_TRIGGER_RE = /\b(\d{4}-\d{2}-\d{2}|Q[1-4]\s+\d{4}|(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+\d{4})\b/i;
 ...
 */
function dedupeItems(candidates, maxItems) {
    const THRESHOLD = 0.7;
    const out = [];
 ...
 */
function extractMacroLeadParagraph(markdown) {
    // Try to find a macro-specific preferred section first.
    const lines = markdown.split(/\r?\n/);
    let heading = '';
    let buffer = [];
    let inFence = false;
    const tryExtract = (buf) => {
    ...
    for (const rawLine of lines) {
        ...
        if (headingMatch) {
            ...
            if (prev &&
                MACRO_PREFERRED_HEADINGS.some((h) => heading
                    .toLowerCase()
 ...
 */
export function extractTopRisks(runDir) {
    const sources = [
 ...
 */
export function extractKeyDates(runDir) {
    const sources = [
        'intelligence/parliamentary-calendar-projection.md',
 ...
 */
export function extractKeyActors(runDir) {
    const sources = [
 ...
 */
export function extractMacroContext(runDir) {
 ...
 */
export function extractKeyTakeaways(runDir) {
    const sources = [
    ...
    if (selected.length < MIN_TAKEAWAYS)
        return [];
    return selected;
}
/**
 ...
 */
function computeSources(runDir) {
    const candidates = [
        'executive-brief.md',
        'extended/executive-brief.md',
        'intelligence/synthesis-summary.md',
 ...
 */
export function serializeArticleMeta(meta) {
