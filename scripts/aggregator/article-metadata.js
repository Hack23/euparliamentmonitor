...
function extractPriorityFindingItem(lines, i) {
    ...
    const taggedHeading = line.match(/^#{3,4}\s+(?:[A-Z]{1,3}-?\d+|\d+(?:\.\d+)*)\s*[:.)·–—-]+\s*(.+)$/u);
    if (taggedHeading) {
...
function cleanPriorityHeadline(raw) {
    ...
    for (let pass = 0; pass < 2; pass++) {
        ...
        text = text.replace(/^(?:CRITICAL|HIGH(?:\s*PRIORITY)?|MEDIUM(?:\s*PRIORITY)?|LOW(?:\s*PRIORITY)?|URGENT|ALERT|PRIORITY)\s*[:—–-]\s*/iu, '');
    }
    // Drop "Trigger 1:" / "Dossier 2:" / "Priority 3:" style prefixes.
    text = text.replace(/^(?:Trigger|Dossier|Priority|Finding|Item|Highlight|Top|Story|Alert|Judgement|Judgment)\s+\d+\s*[:–—-]\s*/iu, '');
    ...
    while (previous !== text) {
        ...
        text = text.replace(/\s*[^\p{L}\p{N}\s]?\s*(?:CRITICAL|HIGH(?:\s*PRIORITY)?|MEDIUM(?:\s*PRIORITY)?|LOW(?:\s*PRIORITY)?)\s*$/iu, '');
