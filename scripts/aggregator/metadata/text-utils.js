...
export function shouldSkipDescriptionLine(line) {
    ...
    if (/^\(?[0-9]{1,2}[.):]\s/u.test(line))
        return true;
    if (/^\(?[a-z][.)]\s/iu.test(line))
        return true;
    ...
    if (/^(that|which|while|whereas|and|but|for|yet|so|nor|or)\s/iu.test(line))
        return true;
