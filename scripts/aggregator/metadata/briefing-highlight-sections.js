...
import { EXTENDED_DESCRIPTION_MAX_LENGTH, shouldSkipDescriptionLine, stripInlineMarkdown, stripLeadingBoldLabel, stripLeadingProseLabel, } from './text-utils.js';
/**
 ...
 */
export function extractFirstSubsectionUnderSection(markdown, sectionNeedles, lang = 'en') {
 ...
 */
export function extractFirstParagraphUnderSection(markdown, sectionNeedles, lang = 'en') {
    const state = newState();
    for (const raw of markdown.split('\n')) {
        const line = raw.trim();
        const kind = classifyLine(line);
 ...
 */
export function extractFirstNumberedItemUnderSection(markdown, sectionNeedles, lang = 'en') {
...
//# sourceMappingURL=briefing-highlight-sections.js.map