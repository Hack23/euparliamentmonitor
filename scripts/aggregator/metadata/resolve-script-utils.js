...
import { deriveHeadlineFromSummary, hasLeakySeoToken, isUsableResolvedTitle, pickFirstNonEmpty, sanitizeTitleCandidate, } from './resolve-helpers.js';
 ...
 */
function dropLeadingFiller(words) {
    let start = 0;
 ...
 */
function buildDistinctiveFragment(rawEnglish, room) {
    if (!rawEnglish)
        return '';
    ...
    if ([...fragment].length < MIN_DISTINCTIVE_FRAGMENT_CHARS)
        return '';
    return fragment;
}
/**
...
export function composeNonLatinDistinctiveTitle(args) {
    ...
    if (!baseLabel || !contentMatchesLocaleScript(baseLabel, lang))
        return '';
    const budget = budgetFor(lang, 'title');
    const prefix = `${baseLabel}: `;
    const room = budget - [...prefix].length;
    if (room < MIN_DISTINCTIVE_FRAGMENT_ROOM)
        return '';
    for (const candidate of englishCandidates) {
        ...
        if ([...composed].length > budget)
            continue;
        if (hasLeakySeoToken(composed))
            continue;
        if (findTitleRejectionReason(composed))
            continue;
        if (!contentMatchesLocaleScript(composed, lang))
            continue;
        return composed;
    }
    return '';
