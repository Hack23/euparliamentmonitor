...
export function localizeArticleBody(bodyHtml, lang) {
    ...
    html = html.replace(new RegExp(`(<h2[^>]*id=["']${TRADECRAFT_SECTION_ID}["'][^>]*>)Tradecraft References(</h2>)`), `$1${escapeHTML(tradecraftHeading)}$2`);
    ...
    html = html.replace(/This article is produced under the <a[^>]*>Hack23 AB<\/a> intelligence tradecraft library\.[^<]*/, `${tradecraftIntro.replace('Hack23 AB', '<a href="https://hack23.com">Hack23 AB</a>')}`);
    ...
    html = html.replace(new RegExp(`(<h2[^>]*id=["']${MANIFEST_SECTION_ID}["'][^>]*>)Analysis Index(</h2>)`), `$1${escapeHTML(analysisIndexHeading)}$2`);
    ...
    const localizedIntroWithLink = manifestUrl
        ...
        : analysisIndexIntro;
    html = html.replace(/Every artifact below was read by the aggregator and contributed to this article\. The raw <a[^>]*>manifest\.json<\/a> carries the full machine-readable list, including gate-result history\./, localizedIntroWithLink);
    ...
    html = html.replace(/<th>Section<\/th>\s*<th>Artifact<\/th>\s*<th>Path<\/th>/, `<th>${escapeHTML(colSection)}</th><th>${escapeHTML(colArtifact)}</th><th>${escapeHTML(colPath)}</th>`);
    // --- Key Takeaways heading ---
    const keyTakeawaysHeading = getLocalizedString(KEY_TAKEAWAYS_HEADING_LABELS, lang);
    html = html.replace(/(<h2[^>]*id=["']section-key-takeaways["'][^>]*>)Key Takeaways(<\/h2>)/, `$1${escapeHTML(keyTakeawaysHeading)}$2`);
    // --- Supplementary Intelligence heading ---
    const supplementaryHeading = getLocalizedString(SUPPLEMENTARY_HEADING_LABELS, lang);
    html = html.replace(/(<h2[^>]*id=["']supplementary-intelligence["'][^>]*>)Supplementary Intelligence(<\/h2>)/, `$1${escapeHTML(supplementaryHeading)}$2`);
    return html;
}
/**
