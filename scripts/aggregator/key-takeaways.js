 ...
 */
function handleHeading(state, headingText) {
 ...
 */
function recordBullet(state, body) {
    if (state.inPreferredSection)
        state.preferred.push(body);
    else if (state.inAnyContent)
        state.fallback.push(body);
}
/**
 ...
 */
export function extractStrongBullets(markdown) {
    const state = {
 ...
 */
export function dedupeTakeaways(candidates) {
    const out = [];
    for (const candidate of candidates) {
        if (out.length >= MAX_TAKEAWAYS)
            break;
        const isDuplicate = out.some((existing) => jaccardSimilarity(existing.body, candidate.body) >= DEDUPE_THRESHOLD);
        if (!isDuplicate)
            out.push(candidate);
    }
    return out;
 ...
 */
export function harvestCandidates(runDir, sources) {
    const candidates = [];
 ...
 */
export function buildKeyTakeaways(options) {
...
//# sourceMappingURL=key-takeaways.js.map